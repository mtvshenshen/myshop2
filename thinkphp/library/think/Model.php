...
abstract class Model implements \JsonSerializable, \ArrayAccess
{
    ...
    public function __construct($data = [])
    {
        ...
        if (is_null($this->autoWriteTimestamp)) {
            // 自动写入时间戳
            $this->autoWriteTimestamp = $this->getBaseQuery()->getConfig('auto_timestamp');
        }

        if (is_null($this->dateFormat)) {
            // 设置时间戳格式
            $this->dateFormat = $this->getBaseQuery()->getConfig('datetime_format');
        }

        if (is_null($this->resultSetType)) {
            $this->resultSetType = $this->getBaseQuery()->getConfig('resultset_type');
        }
        // 执行初始化操作
        $this->initialize();
    }
     ...
     * @return Query
     */
    public function getBaseQuery($buildNewQuery = false)
    {
        if ($buildNewQuery) {
    ...
    {
        $query = $this->getBaseQuery($buildNewQuery);
    ...
    {
        if (!empty($name)) {
            $table = $this->getBaseQuery()->getTable($name);
            return $this->getBaseQuery()->getPk($table);
        } elseif (empty($this->pk)) {
            $this->pk = $this->getBaseQuery()->getPk();
        }
        return $this->pk;
    }
    ...
    public function save($data = [], $where = [], $sequence = null)
    {
        ...
        if ($this->isUpdate) {
            ...
            if (!empty($allowFields)) {
                $result = $this->getBaseQuery()->where($where)->strict(false)->field($allowFields)->update($data);
            } else {
                $result = $this->getBaseQuery()->where($where)->update($data);
            }
            ...
            if (!empty($allowFields)) {
                $result = $this->getBaseQuery()->strict(false)->field($allowFields)->insert($this->data, $this->replace, false, $sequence);
            } else {
                $result = $this->getBaseQuery()->insert($this->data, $this->replace, false, $sequence);
            }

            // 获取自动增长主键
            if ($result && $insertId = $this->getBaseQuery()->getLastInsID($sequence)) {
                foreach ((array) $pk as $key) {
    ...
    {
        if (true === $this->field) {
            $this->field = $this->getBaseQuery()->getTableInfo('', 'fields');
            $field       = $this->field;
            ...
            $fields      = $this->getBaseQuery()->getTableInfo('', 'fields');
    ...
    public function setInc($field, $step = 1, $lazyTime = 0)
    {
        ...
        $result = $this->getBaseQuery()->where($where)->setInc($field, $step, $lazyTime);
        if (true !== $result) {
    ...
    public function setDec($field, $step = 1, $lazyTime = 0)
    {
        // 更新条件
        $where  = $this->getWhere();
        $result = $this->getBaseQuery()->where($where)->setDec($field, $step, $lazyTime);
        if (true !== $result) {
            $this->data[$field] -= $step;
        }
    ...
    public function delete()
    {
        ...
        // 删除当前模型数据
        $result = $this->getBaseQuery()->where($where)->delete();
