...
class Distribution extends BaseApi
{
	...
	public function shopConfig()
	{
	    ...
	    $nfx_shop_config = $shop_config->getShopConfigDetail($this->instance_id);
	...
	public function regionAgentConfig()
	{
		...
		if ($this->nfx_shop_config['is_regional_agent'] == 0) {
			$this->outMessage($title, null, -10, '区域代理功能暂未开启，请联系管理人员');
		}
	...
	public function userConsume()
	{
	    ...
	    $shop_user_account = $shop->getShopUserConsume($this->instance_id, $this->uid);
	...
	public function checkApplyPromoter()
	{
		...
		$shop = new ShopService();
		$uid = $this->uid;
		$user_consume = $shop->getShopUserConsume($this->instance_id, $uid);
		$data = array(
			'reapply' => $reapply,
	...
	function userFxQrcode()
	{
		...
		$path = getQRcode(__URL__ . '/wap/index?source_uid=' . $this->uid, $upload_path, 'shop_goods_qrcode_' . $this->uid);
		...
		$weixin = new Weixin();
		$data = $weixin->getWeixinQrcodeConfig($this->instance_id, $uid, 2);//2-分销店铺二维码
	...
	function showUserQecode($path, $thumb_qrcode, $user_headimg, $shop_logo, $user_name, $data, $create_path)
	{
		//暂无法生成
		if (!file_exists($path)) {
			$path = ROOT_PATH."public/static/images/qr.png";
		}
		$image = \think\Image::open($path);
		$image->thumb(288, 288, \think\Image::THUMB_CENTER)->save($thumb_qrcode);
		$dst = $data["background"];
		if (!file_exists($dst)) {
			$dst = ROOT_PATH."public/static/images/qrcode_bg/qrcode_bg.png";
		}
		...
		imagedestroy($src_im);
		if (!file_exists(ROOT_PATH . $user_headimg)) {
			$user_headimg = "public/static/images/qrcode_bg/head_img.png";
		}
		if (substr($user_headimg, -3) == 'png') {
			$src_im_1 = imagecreatefrompng(ROOT_PATH . $user_headimg);
		} elseif (substr($user_headimg, -3) == 'jpg') {
			$src_im_1 = imagecreatefromjpeg(ROOT_PATH . $user_headimg);
		}
		$src_info_1 = getimagesize(ROOT_PATH . $user_headimg);
		...
		if ($data['is_logo_show'] == '1') {
			if (!file_exists($shop_logo)) {
				$shop_logo = ROOT_PATH."public/static/images/logo.png";
			}
	...
	public function distributionGoodsList()
	{
		...
		if (!empty($uid)) {
			$nfx_promoter = new NfxPromoter();
			$promoter_info = $nfx_promoter->getUserPromoter($uid, $this->instance_id);
