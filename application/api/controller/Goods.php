...
use addons\NsCombopackage\data\service\Promotion as ComboPackagePromotionService;
...
class Goods extends BaseApi
{
	...
	{
		$promotion = new ComboPackagePromotionService();
	...
	public function deleteCart()
	{
		...
		$applet_goods = new GoodsService();
		$res = $applet_goods->cartDelete($cart_id_array);
	...
	public function addCart()
	{
		...
		$retval = $goods->addCart($uid, $this->instance_id, $shop_name, $goods_id, $goods_name, $sku_id, $sku_name, $price, $count, $picture_id, 0);
		return $this->outMessage($title, $retval);
	}
			...
			if ($retval) {
				...
				if ($integralConfig['click_coupon'] == 1) {
					...
					if ($result['click_coupon'] != 0) {
						...
						$member->memberGetCoupon($this->uid, $result['click_coupon'], 2);
					}
				}
			}
			return $this->outMessage($title, $retval);
	...
	public function goodsDiscountList()
	{
		...
		$discount_list = $goods_service->getDiscountGoodsList($page_index, $page_size, $condition, 'end_time');
	...
	public function addGoodsConsult()
	{
		...
		$goods = new GoodsService();
		$retval = $goods->addConsult($goods_id, $goods_name, $this->uid, $member_name, $this->instance_id, $shop_name, $ct_id, $consult_content);
		return $this->outMessage($title, $retval);
	}
	 ...
	 * @return string|\think\response\Json
	 */
	public function updateGoodsClicks()
	{
		$title = "更新商品点击量";
		$goods_id = isset($this->params['goods_id']) ? $this->params['goods_id'] : 0;
		if (empty($goods_id)) {
		...
		$goods = new GoodsService();
		$res = $goods->updateGoodsClicks($goods_id);
		return $this->outMessage($title, $res);
	}
	...
	{
		$category_id = isset($this->params['category_id']) ? $this->params['category_id'] : ''; // 商品分类
		$keyword = isset($this->params['keyword']) ? $this->params['keyword'] : ''; // 关键词
		$shipping_fee = isset($this->params['shipping_fee']) ? $this->params['shipping_fee'] : ''; // 是否包邮，0：包邮；1：运费价格
		$stock = isset($this->params['stock']) ? $this->params['stock'] : ''; // 仅显示有货，大于0
		$page_index = isset($this->params['page_index']) ? $this->params['page_index'] : 1; // 当前页
		$order = isset($this->params['order']) ? $this->params['order'] : ''; // 排序字段,order by ziduan
		$sort = isset($this->params['sort']) ? $this->params['sort'] : ''; // 排序方式
		$brand_id = isset($this->params['brand_id']) ? $this->params['brand_id'] : ''; // 品牌id
		$brand_name = isset($this->params['brand_name']) ? $this->params['brand_name'] : ''; // 品牌名牌
		$min_price = isset($this->params['min_price']) ? $this->params['min_price'] : ''; // 价格区间,最小
		$max_price = isset($this->params['max_price']) ? $this->params['max_price'] : ''; // 最大
		$province_id = isset($this->params['province_id']) ? $this->params['province_id'] : ''; // 商品所在地
		$province_name = isset($this->params['province_name']) ? $this->params['province_name'] : ''; // 所在地名称
		$attr = isset($this->params['attr']) ? $this->params['attr'] : ''; // 属性值
		$spec = isset($this->params['spec']) ? $this->params['spec'] : ''; // 规格值
		$platform_proprietary = isset($this->params['platform_proprietary']) ? $this->params['platform_proprietary'] : ''; // 平台自营 shopid== 1
		$page_size = isset($this->params['page_size']) ? $this->params['page_size'] : PAGESIZE;
	...
	{
		$member = new MemberService();
		$guess_member_likes = $member->getGuessMemberLikes();
	...
	public function goodsCategoryBlockPc()
	{
		...
		$block_list = $shop->getGoodsFloorList(1, 0, [ 'is_use' => 1 ], 'sort', '*');
	 ...
	 * 创建时间：2018年12月28日16:58:44
	 */
	public function goodsCategoryBlockWap()
	{
		$show_num = isset($this->params['show_num']) ? $this->params['show_num'] : 4;
		$shop = new ShopService();
