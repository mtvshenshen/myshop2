...
class Pay extends BaseWap
{
	...
	public function __construct()
	{
		parent::__construct();
		$config = new Config();
		$this->assign("shopname", $this->web_info['title']);
	...
	public function wchatPay()
	{
		...
		if (!isWeixin()) {
			...
			if (request()->isMobile()) {
				...
				$this->assign('pay_value', $pay_value);
				return $this->view($this->style . "pay/pc_wechat_pay");
			}
		} else {
			// jsapi支付
			...
			if (!empty($res["return_code"]) && $res["return_code"] == "FAIL" && $res["return_msg"] == "JSAPI支付必须传openid") {
				...
				return $this->view($this->style . 'pay/weixin_pay');
			}
		}
	}
	...
	public function aliPayReturn()
	{
		...
		$pay = new UnifyPay();
		$params = request()->get();
		$verify_result = $pay->getVerifyResult($params, 'return');
		if ($verify_result) {
			$trade_no = request()->get('trade_no', '');
			$trade_status = request()->get('trade_status', '');
			$pay_config = new AlipayConfig();
			$edition = $pay_config->getAliPayVersion(0);
			$ali_edition = json_decode($edition['value'], true);
			if(empty($edition) || $ali_edition['is_use'] == 0){
				if ($trade_no == 'TRADE_FINISHED' || $trade_status == 'TRADE_SUCCESS') {
					// return $this->view($this->style . 'Pay/pay_success');
					// echo "<h1>支付成功</h1>";
					$this->assign("status", 1);
					if (request()->isMobile()) {
	...
	public function getOrderNoByOutTradeNo($out_trade_no)
	{
		...
		if ($pay_value['type'] == 1) {
			// 订单
			$list = $order_query->getOrderNoByOutTradeNo($out_trade_no);
			if (!empty($list)) {
				foreach ($list as $v) {
					$order_no .= $v['order_no'];
				}
