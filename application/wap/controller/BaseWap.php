...
class BaseWap extends Controller
{
	...
	public function __construct()
	{
		...
		if (!empty($this->app_login_name) && !empty($this->app_login_password)) {
			$this->setWapNewPassWordByEmailOrMobile = new Member();
			$this->user->login($this->app_login_name, $this->app_login_password);
		}
	...
	public function initInfo()
	{
		...
		$this->web_info = $this->web_site->getWebSiteInfo();
		...
		$this->share_icon = $this->web_info['web_wechat_share_logo'];
		...
		if (!request()->isAjax()) {
			...
			////主要想实现，没有配置微信公众号的情况下，手机端也能正常访问，
			if (!empty($wchat_config['value']['appid']) && !empty($wchat_config['value']['appid'])) {
				if (empty($this->uid) && empty(Session::get('is_reload'))) {
					$this->wchatLogin();
				}
			}
			$this->assign("uid", $this->uid);
		...
		if (empty($goods_id)) {
			switch ($flag) {
				...
				case "qrcode_shop":
					...
					$user = new User();
					$user_info = $user->getUserInfoByUid($this->uid);
					$share_url = __URL(__URL__ . '/wap/login/qrcodeshop?source_uid=' . $this->uid);
					$current_user = "分享人：" . $user_info["nick_name"];
					break;
				case "qrcode_my":
					...
					$user = new User();
					$user_info = $user->getUserInfoByUid($this->uid);
					$share_url = __URL(__URL__ . '/wap/login/qrcode?source_uid=' . $this->uid);
					$current_user = "分享人：" . $user_info["nick_name"];
					break;
				case "bargain":
