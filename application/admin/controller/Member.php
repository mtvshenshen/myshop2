...
class Member extends BaseController
{
	...
	public function memberList()
	{
		...
		if (request()->isAjax()) {
			...
			$level_id = request()->post('levelid', 'all');
	...
	public function shopAgree()
	{
		...
		if (request()->isAjax()) {
			...
			$condition = [
				...
				'su.user_shop|su.user_shop_agree' => 1,
			];
			...
			return view($this->style . 'Member/shopAgree');
		}
	}	
	...
	public function updateMember()
	{
		if (request()->isAjax()) {
			...
			$res = $member->updateMemberByAdmin($uid, $user_name, $email, $sex, $status, $mobile, $nick_name, $member_level, $member_label);
	 ...
	 * 修改分成比例
	 */
	public function updatefen()
	{

		$userid = request()->post('uid', '');
		$user_fen = request()->post('user_fen', '');
		Db::execute('update sys_user set user_shop_fen ='.$user_fen.' where uid='.$userid);
	...
	public function addMember()
	{
		...
		$sex = request()->post('sex', '');
		$status = request()->post('status', '');
		$retval = $member->addMember($user_name, $password, $email, $sex, $status, $mobile, $member_level, $member_label);
	...
	{
		$uid = request()->post('id', '');
		$user_shop = request()->post('user_shop', '');
		$retval = 0;
	...
	public function memberLevelList()
	{
		...
		if (request()->isAjax()) {
			$page_index = request()->post("page_index", 1);
			$page_size = request()->post('page_size', PAGESIZE);
			$list = $member->getMemberLevelList($page_index, $page_size, [], 'level');
			return $list;
		}
		$config = new Config();
	...
	public function memberDataExcel()
	{
		...
		$label_id = request()->get('label_id', -1);
		$condition = [
			'su.is_member' => 1,
			'su.nick_name|su.user_tel|su.user_email' => array(
				'like',
		...
		if ($label_id != -1) {
			$condition['nm.member_label'] = $label_id;
		}
		$member = new MemberService();
	...
	public function memberExcellist()
	{
		...
		$label_id = request()->post('label_id', -1);
	...
	public function updateMemberLabel()
	{
	    $member_label = request()->post('member_label', '');
	    $uid = request()->post('uid', '');
	    if(empty($uid) || empty($member_label)) return -1;
	    $retval = 0;
