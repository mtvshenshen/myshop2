...
class Config extends BaseController
{
	...
	public function wchatConfig()
	{
		...
		if (request()->isAjax()) {
			...
			$res_one = $wxpay_config->setWpayConfig($this->instance_id, $appkey, $appsecret, $MCHID, $paySignKey, $is_use);
	...
	public function alipayConfig()
	{
		...
		if (request()->isAjax()) {
			...
			$is_use = request()->post('is_use', 0);
			// 获取数据
			$res_one = $alipay_config->setAlipayConfig($this->instance_id, $partnerid, $seller, $ali_key, $is_use);
	 ...
	 *
	 */
	public function classifiedDisplayMode()
	{
		// 分类显示方式
		$shop_id = $this->instance_id;
		$web_config = new WebConfig();
		$info = $web_config->getWapClassifiedDisplayMode($shop_id);
		$this->assign('info', $info);
		return view($this->style . 'Config/classifiedDisplayMode');
	}
	 ...
	 * @return Ambigous <multitype:unknown, multitype:unknown unknown string >|Ambigous <\think\response\View, \think\response\$this, \think\response\View>
	 */
	public function updateWapClassifiedDisplayMode()
	{
		$web_config = new WebConfig();
		$shopid = $this->instance_id;
		if (request()->isAjax()) {
			$classified_display_mode = request()->post("data", 1);
			$res = $web_config->setWapClassifiedDisplayMode($shopid, $classified_display_mode);
			return AjaxReturn($res);
		}
	}
	...
	public function apiSecure()
	{
		...
		if (request()->isAjax()) {
			$data = request()->post("data", "");
			$res = $web_config->setApiSecureConfig($this->instance_id, $data, 1);
			return AjaxReturn($res);
		} else {
			$api_secure = $web_config->getApiSecureConfig($this->instance_id);
	...
	public function goodsRecommend()
	{
		if (request()->isAjax()) {
			...
			if ($recommend_id) {
				//编辑
				$res = $shop->updateGoodsRecommend($recommend_id, $alis_id, $type, $recommend_name, $num);
			} else {
				//添加
				$res = $shop->addGoodsRecommend($alis_id, $type, $recommend_name, $num);
			}
			return AjaxReturn($res);
		}
...
// 	        $this->assign("goods_id_array", $goods_id_array);
			$shop = new Shop();
			$list = $shop->getGoodsRecommend(1, 0, [ 'shop_id' => $this->instance_id ]);
			$this->assign("recommend_list", $list);
	...
	public function delgoodsRecommend()
	{
		if (request()->isAjax()) {
			$id = request()->post('id', 0);
			if (!$id) return -1;
			$shop = new Shop();
			$res = $shop->delectGoodsRecommend($id);
			return $res;
		}
	}
	...
	public function saveRecommendImg()
	{
		if (request()->isAjax()) {
			...
			if (!$id) return -1;
			$shop = new Shop();
			$res = $shop->saveRecommendImg($id, $img);
			return AjaxReturn($res);
		}
	}
	...
	public function wapBottomType()
	{
		...
		if (request()->isAjax()) {
			...
			$res = $web_config->setWapBttomType($data);
			return AjaxReturn($res);
		}
		...
		$info = $web_config->getWapBttomType($this->instance_id);
		$info['data'] = json_decode($info['template_data'], true);
		$this->assign('link_arr', $link_arr);
		
		//展示页面
		$show_page = [
			'APP_MAIN/index/index' => '首页',
			'APP_MAIN/goods/category' => '商品分类页',
			'APP_MAIN/member/index' => '会员中心',
			'APP_MAIN/goods/cart' => '购物车',
			'APP_MAIN/goods/groupbuy' => '团购专区',
			'APP_MAIN/goods/topic' => '专题活动',
			'APP_MAIN/goods/brand' => '品牌专区',
			'APP_MAIN/article/lists' => '文章中心',
		];
	...
	public function wapStyle()
	{
		...
		if (request()->isAjax()) {
			...
			$res = $web_config->setWapStyle($data);
			return AjaxReturn($res);
		}
		
		$info = $web_config->getWapStyle($this->instance_id);
		$info['data'] = json_decode($info['template_data'], true);
		...
		$style_arr = [
			...
			1 => [
				'main_color' => '#fcc600',
				'secondary_color' => '#333',
				'img' => 'style-img-fcc600.png',
			],
			2 => [
				'main_color' => '#fd4444',
				'secondary_color' => '#333',
				'img' => 'style-img-fd4444.png',
			],
			3 => [
				'main_color' => '#64c5aa',
				'secondary_color' => '#333',
				'img' => 'style-img-64c5aa.png',
			],
			4 => [
				'main_color' => '#333333',
				'secondary_color' => '#333',
				'img' => 'style-img-333333.png',
			],
			5 => [
				'main_color' => '#63be71',
				'secondary_color' => '#333',
				'img' => 'style-img-63be71.png',
			],
		];
		$this->assign('style_arr', $style_arr);
		
		$this->assign('info', $info);
		return view($this->style . "Config/wapStyle");
	}
	...
	public function setstatus()
	{
		...
		if ($type == "WCHAT") {
			$wchat_config = $web_config->setWchatConfigIsuse($this->instance_id, $is_use);
			return AjaxReturn($wchat_config);
		} else if ($type == "QQLOGIN") {
			$qq_config = $web_config->setQQConfigIsuse($this->instance_id, $is_use);
