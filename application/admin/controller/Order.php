...
// 		}
	...
	public function getOrderInvoiceView()
	{
		...
		// 操作
		$order_query = new OrderQuery();
		$goods_express_list = $order_query->getOrderGoodsExpressDetail($orderIdArray, $shop_id);
		// 返回信息
	...
	public function getOrderExpressPreview()
	{
		...
		// 操作
		$order_query = new OrderQuery();
		$goods_express_list = $order_query->getOrderPrint($orderIdArray, $shop_id);
		// 返回信息
		return $goods_express_list;
	}
	...
	public function printDeliveryPreview()
	{
		...
		$order_query = new OrderQuery();
		$receive_address = $order_query->getShopReturnSet($shop_id);
	...
	public function orderOffLinePay()
	{
		...
		$order_info = $order_query->getOrderInfo($order_id);
	...
	public function orderDelivery()
	{
		...
		if ($shipping_type == 1) {
			$res = $order_action->orderDelivery($order_id, $order_goods_id_array, $express_name, $shipping_type, $express_company_id, $express_no);
		} else {
	...
	public function orderGoodsRefundMoney()
	{
		...
		$res = 0;
		if ($order_goods_id != '') {
			$res = $order_query->orderGoodsRefundMoney($order_goods_id);
		}
		return $res;
	}
	...
	public function returnSetting()
	{
		...
		if (request()->isAjax()) {
			...
			$retval = $order_refund->updateShopReturnSet($shop_id, $address, $real_name, $mobile, $zipcode);
			return AjaxReturn($retval);
		} else {
		    $order_query = new OrderQuery();
			$info = $order_query->getShopReturnSet($shop_id);
	...
	public function addMemo()
	{
		...
		$result = $order_action->addOrderSellerMemo($order_id, $memo);
	...
	public function updateOrderAddress()
	{
		...
		$res = $order_action->updateOrderReceiveDetail($order_id, $receiver_mobile, $receiver_province, $receiver_city, $receiver_district, $receiver_address, $receiver_zip, $receiver_name, $fixed_telephone);
		return $res;
	}
	...
	public function getOrderGoodsDetialAjax()
	{
		if (request()->isAjax()) {
			$order_goods_id = request()->post("order_goods_id", '');
			$order_query = new OrderQuery();
			$res = $order_query->getOrderGoodsRefundDetail($order_goods_id);
			return $res;
		}
	}
	...
	public function deleteOrder()
	{
		if (request()->isAjax()) {
			$order_action = new OrderAction();
			$order_id = request()->post("order_id", "");
			$res = $order_action->deleteOrder($order_id, 1, $this->instance_id);
			return AjaxReturn($res);
		}
	}
	...
	public function getOrderTermsOfPayment()
	{
		...
		if (!empty($order_id)) {
			...
			$type = $order_query->getPayType(["pay_type" => $payment_type]);
			$json = array();
			if ($type == "微信支付") {
				$temp['type_id'] = 1;
				$temp['type_name'] = "微信";
...
//			)
	...
	public function updateOrderExpress()
	{
		...
		$express_company_id = request()->post('express_company_id', '');
		$express_no = request()->post('express_no', '');
		$res = $order_action->updateDelivey($order_goods_express_id, $express_name, $shipping_type, $express_company_id, $express_no);
		return AjaxReturn($res);
	}
