...
class Block extends BaseController
{
    ...
    public function edit()
    {
        ...
        if (request()->isAjax()) {
            ...
            if (!$id) {
                ...
                $res = $shop->setGoodsFloor($block_data);
            } else {
                $block_data['modify_time'] = time();
                $res = $shop->setGoodsFloor($block_data, ['id' => $id]);;
            }
    ...
    public function loadBlock($data, $block_template)
    {
        ...
        if (!empty($this->siteInfo['pc_template'])) {
            $pc_template = json_decode($this->siteInfo['pc_template'], true);
            $style = $pc_template['style'];
        }
        if (!empty($data)) {
            $data = json_decode($data, true);
	...
	public function modifyFloorSort()
	{
	    if (request()->isAjax()) {
	        ...
	        $retval = $shop->updateFloorSort($sort, $id);
