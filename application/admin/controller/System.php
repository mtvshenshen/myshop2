...
class System extends BaseController
{
	...
	public function addShopAdvPosition()
	{
	    if (request()->isAjax()){
	        ...
	        $platform = new Platform();
	        $res = $platform -> addOrEditAdvPosition($value);
	        return $res;
	    }
	...
	public function updateShopAdvPosition()
	{
		...
		$info = $platform->getAdvPositionDetail(['ap_id' => $id]);
		if (empty($info)) {
			$this->error('未获取到信息');
		}
	...
	public function check_apkeyword()
	{
		...
		if (request()->isAjax()) {
			...
			$res = 0;
			if ($ap_keyword != "") {
				$res = $platform->check_apKeyword_is_exists($ap_keyword, $ap_id);
			}
			return $res;
		}
	}
	 ...
	 * 友情链接的是否新窗口打开
	 */
	public function setPlatformLinklistIsblank()
	{
	    if (request()->isAjax()) {
	        $link_id = request()->post('link_id', '');
	        $is_show = request()->post('is_show', '');
	        $platform = new Platform();
	        $res = $platform->setPlatformLinklistIsblank($link_id, $is_show);
// 	        Cache::tag("niu_platform_adv_position")->set("getPlatformAdvPositionDetail" . $ap_id, '');
	        return AjaxReturn($res);
	    }
	}
	 ...
	 * 友情链接的是否显示
	 */
	public function setPlatformLinklistIsshow()
	{
	    if (request()->isAjax()) {
	        $link_id = request()->post('link_id', '');
	        $is_use = request()->post('is_use', '');
	        $platform = new Platform();
	        $res = $platform->setPlatformLinklistIsshow($link_id, $is_use);
	        // 	        Cache::tag("niu_platform_adv_position")->set("getPlatformAdvPositionDetail" . $ap_id, '');
	        return AjaxReturn($res);
	    }
	}
