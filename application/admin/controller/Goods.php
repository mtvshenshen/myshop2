...
class Goods extends BaseController
{
	...
	public function editGoods(){
	    ...
	    //加载初始信息
	    $this->goodsAssign($goods_id);
	    
	    $res = hook('editGoods', ['type' => $type, 'goods_id' => $goods_id]);
	    $res = arrayFilter($res);
	    if(empty($res[0]))
	    {
	        $this->error("商品类型不存在");
	...
	public function GoodsCreateOrUpdate(){
		...
		if (request()->isAjax()) {
    	    ...
    	    $goodservice = new GoodsService();
    	    $retval = $goodservice->addOrEditGoods($data);
    	    ...
    	    if (empty($data['goods_id'])) {
    	        ...
    	        $goodservice->goods_QRcode_make($data['goods_id'], $pay_qrcode);
    	    }
