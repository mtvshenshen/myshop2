...
function getApiData($method, $params)
{
	...
	$api_config = $config->getApiSecureConfig(0);
	if (!empty($api_config)) {
 ...
 * @return array
 */
function getDayStep($end_time, $srart_time){
    
    $num = ($end_time - $srart_time) / (3600 * 24);
    if($num < 0){
        $num = 1;
    }
    $step_name = " day";
    ...
    $data = [];
    for ($i = 0; $i < $num; $i++) {
        $data[date($format, strtotime("+ " .$i.$step_name, $srart_time))] = 0;
    }
    return $data;
}
