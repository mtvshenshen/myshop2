...
function loadInfo(page) {
	...
	api('System.Member.footprint', {"page_index": page, "page_size": 14, category_id: category_id}, function (res) {
		console.log(res);
		if (res.code == 0) {
			...
			if (list.length > 0) {
				$('.line-hight,.line').removeClass('hide');
				for (var i = 0; i < list.length; i++) {
					...
					if (!$('#' + item.add_date).length) {
						var html = `<div class="item" id="` + item.add_date + `">
									...
									<div class="tit">
										<time>` + item.add_date + `</time><a onclick="delFootprint('add_date', '` + item.add_date + `')" class="del">删除</a>
									</div>
									<div class="cont">
										<ul class="goodslist clearfix">
											<li>
												...
												<a href="` + __URL(SHOPMAIN + '/goods/detail?goods_id=' + item.goods_id) + `" class="goods-img" title="` + item.goods_info.goods_name + `">
													<img src="` + __IMG(item.goods_info.picture_info.pic_cover_mid) + `" alt="">
												</a>
