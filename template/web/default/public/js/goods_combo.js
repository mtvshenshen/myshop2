...
const real_price = $("#realprice").text();
...
$("#buy_num").change(function () {
	...
	for (var i = 0; i < stock_arr.length; i++) {
		if (num > stock_arr[i]) {
			$("#js-immediate-purchase").removeClass("btn-jiesuan").removeClass("ns-bg-color").addClass("btn-jiesuan-disabled").addClass("ddd").attr("disabled", true).css("pointer-events", "none");
			break;
		} else {
			$("#js-immediate-purchase").removeClass("btn-jiesuan-disabled").removeClass("ddd").addClass("btn-jiesuan").addClass("ns-bg-color");
		}
	}
	CalculatePrice();
});
