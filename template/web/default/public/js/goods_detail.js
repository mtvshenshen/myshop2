$(function () {
	...
	$(".sku-list .item-line li:not(.disabled)").click(function () {
		...
		location.href = __URL(SHOPMAIN + "/goods/detail?sku_id=" + sku_id + "&goods_id=" + goods_id);
	});
	...
	api("System.Goods.goodsConsultList", {page_size: 5, goods_id: goods_id, ct_id: ct_id}, function (res) {
		...
		if (data) {
			...
			if (data.total_count) {
				...
				h += '<div class="empty">' + lang_goods_detail.goods_no_consultation_ye + '</div>';
				$(".js-consult .empty").html(h).parent().show();
			}
...
function coupon_receive(event, coupon_type_id) {
	if (uid == '') {
		...
		if (data_max_fetch != 0 && data_receive_quantity >= data_max_fetch) {
			show(lang_goods_detail.reached_the_limit);
			return false;
		}
		api('System.Goods.receiveGoodsCoupon', {"coupon_type_id": coupon_type_id, 'scenario_type': 3}, function (res) {
			var data = res.data;
			if (data > 0) {
				$(event).attr("data-receive-quantity", data_receive_quantity + 1);
		...
		if (data.data.length > 0) {
			$.each(data.data, function (i, e) {
				...
				if (e.user_img != "" && e.user_img != undefined && e.user_img != 0) {
					html += '<img src="' + __IMG(e.user_img) + '" width="25" height="25" alt="">';
				} else {
					html += '<img src="' + __IMG($("#hidden_default_headimg").val()) + '" width="25" height="25" alt="">';
				}
				...
				if (e.image != '') {
					...
					$.each(img, function (index, vo) {
						html += '<a href="' + __IMG(vo) + '" data-toggle="lightbox" data-group="image-group-1"><img src="' + __IMG(vo) + '" class="ns-border-color-hover" width="48" height="48"></a>';
					});
				}
			...
			html += ' <ul id="evaluate_page" class="pager" data-elements="prev,nav,next,total_page_text,goto" <ul data-rec-per-page="' + page_size + '" data-ride="pager" data-page="' + page_index + '" data-rec-total="' + data.total_count + '"></ul>';
		} else {
