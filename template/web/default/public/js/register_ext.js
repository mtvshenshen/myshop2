...
$(function () {
	...
	lang(["user_name_already_exists", "password_must_contain_symbols", "password_must_have_uppercase_letters", "password_must_have_lowercase_letters", "password_must_contain_numbers", "password_cannot_includ_chinese_characters", "minimum_password_length", "password_cannot_empty", "such_characters", "username_cannot_includ", "user_name_canno_be_phone", "user_name_canno_be_mailbox", "two_password_not_same", "user_name_length"], function (res) {
		login_data = res;
	});
		...
		if ($(this).val().length >= 3 && $(this).val().length <= 16) {
			...
			api("System.Member.checkUsername", {"username": user_name}, function (res) {
				...
				if (data) {
					...
					show(login_data.user_name_already_exists);
...
function perfectInfo() {
	...
	if (!(username.length >= 3 && username.length <= 16)) {
		...
		show(login_data.user_name_length);
		return false;
	} else {
		$("#username").removeClass("ns-border-color");
	}
	...
	if (!(repass == password)) {
		$("#repass").trigger("focus").addClass("ns-border-color");
		show(login_data.two_password_not_same);
		return false;
	} else {
		$("#repass").removeClass("ns-border-color");
	}
...
function verifyUsername(username) {
	...
	if (regex.email.test(username)) {
		is_true = 1;
		$("#username").trigger("focus").addClass("ns-border-color");
		show(login_data.user_name_canno_be_mailbox);
		return is_true;
	}
	if (regex.mobile.test(username)) {
		is_true = 1;
		$("#username").trigger("focus").addClass("ns-border-color");
		show(login_data.user_name_canno_be_phone);
		return is_true;
	}
	...
	$.each(usernme_verify_array, function (k, v) {
		if (username.indexOf(v) >= 0) {
			is_true = 1;
			$("#username").trigger("focus").addClass("ns-border-color");
			show(login_data.username_cannot_includ + v + login_data.such_characters);
			return false;
		}
	});
	return is_true;
...
function verifyPassword(password) {
	...
	if ($.trim(password) == "") {
		is_true = 1;
		$("#pass").trigger("focus").addClass("ns-border-color");
		show(login_data.password_cannot_empty);
		return is_true;
	} else {
		$("#pass").removeClass("ns-border-color");
	}
	if (min_length > 0) {
		if (password.length < min_length) {
			is_true = 1;
			$("#pass").trigger("focus").addClass("ns-border-color");
			show(login_data.minimum_password_length + min_length);
			return is_true;
		} else {
			$("#pass").removeClass("ns-border-color");
		}
	...
	if ($.trim(regex_str) != "" && regex_str != undefined) {
		...
		if (regex_str.indexOf("number") >= 0) {
			...
			if (!number_test.test(password)) {
				is_true = 1;
				$("#pass").trigger("focus").addClass("ns-border-color");
				show(login_data.password_must_contain_numbers);
				return is_true;
			} else {
				$("#pass").removeClass("ns-border-color");
			}
		...
		if (regex_str.indexOf("letter") >= 0) {
			...
			if (!letter_test.test(password)) {
				is_true = 1;
				$("#pass").trigger("focus").addClass("ns-border-color");
				show(login_data.password_must_have_lowercase_letters);
				return is_true;
			} else {
				$("#pass").removeClass("ns-border-color");
			}
		...
		if (regex_str.indexOf("upper_case") >= 0) {
			...
			if (!upper_case_test.test(password)) {
				is_true = 1;
				$("#pass").trigger("focus").addClass("ns-border-color");
				show(login_data.password_must_have_uppercase_letters);
				return is_true;
			} else {
				$("#pass").removeClass("ns-border-color");
			}
		...
		if (regex_str.indexOf("symbol") >= 0) {
			...
			if (!symbol_test.test(password)) {
				is_true = 1;
				$("#pass").trigger("focus").addClass("ns-border-color");
				show(login_data.password_must_contain_symbols);
				return is_true;
			} else {
				$("#pass").removeClass("ns-border-color");
			}
...
function loginBind() {
	...
	if (vertify(binding_username, binding_pass, binding_verify_code)) {
		...
		api("System.Login.bindAccount", {
			...
			if (res.data > 0) {
				setTimeout(function () {
					location.href = __URL(SHOPMAIN + '/member/index');
				}, 1000)
			} else {
				flag = false;
			}
