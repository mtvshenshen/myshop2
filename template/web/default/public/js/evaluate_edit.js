...
$(function(){
	...
	$('.evaluation-block .evaluation-product-item').each(function(index){
		...
		uploaderArr[index].on('fileQueued', function (file) {
			...
			uploaderArr[index].makeThumb(file, function (error, src) {
				...
				var liLength = _this.find('.updata-item-list .file-item').length;
				
				if(liLength <= 4){
					_this.find('.updata-item-list').append(html);
	...
	$('body').on('click', '.evaluation-product-item .cancel', function(){
		...
		var index = $('.evaluation-block .evaluation-product-item').index($(this).parents('.evaluation-product-item')),
			img_index = $('.evaluation-product-item .updata-item-list .file-item').index($(this).parents('.file-item')),
		...
		if(parentsEl.find('.updata-item-list .file-item').length <= 4){
			parentsEl.find('[class^="head-img"]').removeClass('hide');
			parentsEl.find('.updata-count').text(parentsEl.find('.updata-item-list .file-item').length);
			parentsEl.find('.updata-surplus').text(5 - parentsEl.find('.updata-item-list .file-item').length);
		}
	})
...
function doSubmit(type){
	...
	$('.evaluation-block .evaluation-product-item').each(function(index){
		...
		if(type == 1){
			...
			if(scores == 1) { var explain_type = 3; }
			if(scores >1 && scores <4) { var explain_type = 2; }
			if(scores >3 && scores <6) { var explain_type = 1; }
	...
	api(ajaxUrl,{ "goods_evaluate": JSON.stringify(goodsEvaluate), "order_id": order_id, "order_no": order_no },function (res) {
		if(res.data == 1){
			show('评价成功');
