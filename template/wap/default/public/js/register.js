...
$(function() {
	...
	langApi(['account_cannot_be_empty','password_cannot_empty','get_dynamic_code','confirm_password_can_not_be_empty','two_password_input_is_inconsistent', 'verification_code_cannot_be_null','phone_number_cannot_empty','mobile_phone_number_is_wrong','mobile_phone_is_registered','verification_code_cannot_be_null','two_password_input_is_inconsistent'	,'mobile_phone_dynamic_password_can_not_be_empty'	,'mobile_phone_dynamic_code_error','get_dynamic_code','user_name_cannot_contain_chinese_characters','user_name_canno_be_mailbox','user_name_canno_be_phone','username_cannot_includ','such_characters','minimum_password_length','password_cannot_includ_chinese_characters','password_must_contain_numbers','password_must_have_lowercase_letters','password_must_have_uppercase_letters','password_must_contain_symbols', 'agreement_registration_agreement'],function(res){
		lang= res;
	});
	...
	function time() {
		if (wait == 0) {
			$("#sendOutCode").removeAttr("disabled").val(lang.get_dynamic_code);
			wait = 120;
		} else {
	...
	$("#sendOutCode").click(function (){
		...
		if(mobile.search(regex.mobile) == -1){
			...
			toast(lang.mobile_phone_number_is_wrong);
			return false;
		}
		//验证手机号是否已经注册
		api("System.Member.checkMobile",{ mobile : mobile },function (res) {
			var data = res.data;
			if(data){
				toast(lang.mobile_phone_is_registered);
			}else{
				//判断输入的验证码是否正确
...
function register_member(){
	...
	if(cfpassword == ''){
		toast(lang.confirm_password_can_not_be_empty);
		return false;
	}
	if(password != cfpassword){
		toast(lang.two_password_input_is_inconsistent);
		$("#cfpassword").focus();
		return false;
	}
	if(register_captcha == ''){
	...
	if(register_captcha != '' && register_captcha != undefined){
	...
	if(protocol == 0) {
		toast(lang.agreement_registration_agreement);
		return false;
	}
...
function register_mobile(){
	...
	if(verify_code == ''){
		toast(lang.mobile_phone_dynamic_password_can_not_be_empty);
		return false;
	}
	if(protocol == 0) {
		toast(lang.agreement_registration_agreement);
	...
	api("System.Login.mobileRegister",{ "mobile" : mobile, "password" : password_mobile, 'mobile_code' : verify_code},function (res) {
		var data = res.data;
		if(data.code > 0 ){
			$.ajax({
				type: 'post',
...
function verifyUsername(username){
	...
	if(/.*[\u4e00-\u9fa5]+.*$/.test(username) ){
		is_true = 1;
		toast(lang.user_name_cannot_contain_chinese_characters);
		return is_true;
	}
	if(regex.email.test(username) ){
		is_true = 1;
		toast(lang.user_name_canno_be_mailbox);
		return is_true;
	}
	if(regex.mobile.test(username) ){
		is_true = 1;
		toast(lang.user_name_canno_be_phone);
		return is_true;
	}
	...
	$.each(usernme_verify_array,function(k,v){
		if($.trim(v) != ""){
			if(username.indexOf(v) >= 0){
				is_true = 1;
				toast(lang.username_cannot_includ+v+lang.such_characters);
				return false;
			}
		}
	});
...
function verifyPassword(password){
	...
	if(min_length  > 0){
		if(password.length < min_length){
			is_true = 1;
			toast(lang.minimum_password_length+min_length);
			return is_true;
		}
	}
	if(/.*[\u4e00-\u9fa5]+.*$/.test(password) ){
		is_true = 1;
		toast(lang.password_cannot_includ_chinese_characters);
		return is_true;
	}
	if($.trim(regex_str) != "" && regex_str != undefined){
		...
		if(regex_str.indexOf("number") >= 0){
			...
			if(!number_test.test(password)){
				is_true = 1;
				toast(lang.password_must_contain_numbers);
				return is_true;
			}
		}
		//验证是否包含小写字母
		if(regex_str.indexOf("letter") >= 0){
			...
			if(!letter_test.test(password)){
				is_true = 1;
				toast(lang.password_must_have_lowercase_letters);
				return is_true;
			}
		}
		//验证是否包含大写字母
		if(regex_str.indexOf("upper_case") >= 0){
			...
			if(!upper_case_test.test(password)){
				is_true = 1;
				toast(lang.password_must_have_uppercase_letters);
				return is_true;
			}
		}
		//验证是否包含特殊字符
		if(regex_str.indexOf("symbol") >= 0){
			...
			if(!symbol_test.test(password)){
				is_true = 1;
				toast(lang.password_must_contain_symbols);
				return is_true;
			}
		}
	}
