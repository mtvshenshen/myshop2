		...
		if (is_hide) {
			...
			// 删除操作
				if ($(this).attr("is_del") == 'no') {
					...
					$(".btn.btn-buy").addClass("ns-bg-color");
					var count = 0;
					var select_count = 0;
					...
					if (count == select_count) {
						...
						$(".btn.btn_buy").addClass("ns-bg-color");
					}
				} else {
					...
					if (is_dis == 'no') {
						$(".btn.btn_buy").removeClass("ns-bg-color").css("background", "#CCCCCC")
					} else {
						$(".btn.btn-buy").addClass("ns-bg-color");
					}
					if(count == select_count){
	...
	$("#div_selected").click(function() {
		...
		if (flag) {
			...
			if (is_del == 'no') {
				...
				$(".btn.btn-buy").removeClass("ns-bg-color").css("background", "#CCCCCC");
			}
		} else {
			// 结算
			var temp_src = "";//存放图片路径
...
function cart_edit(obj, shop_id) {
	...
	if (count > 0) {
		...
		$("#select_all").attr("is_check", "no");
		$("#select_all").attr("is_del", "no");
		$(".btn.btn-buy").css("background", "#CCCCCC");
		$("#sel_text").css("color", "#CCCCCC");
		$("#settlement").text("删除");
		//初始化
		$(".checkbox").each(function() {
