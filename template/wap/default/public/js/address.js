function saveAddress() {
    ...
    if(!address_id){
        parmas = {"consigner":name,"mobile":mobile,"province":province,"city":city,"district":district,"address":addressinfo,"phone":phone};
        ajax_url = "addAddress";
    }else{
        parmas = {"id":address_id,"consigner":name,"mobile":mobile,"province":province,"city":city,"district":district,"address":addressinfo,"phone":phone};
    ...
    api("System.Member." + ajax_url, parmas, function(data){
        ...
        if (txt > 0) {
            if(flag == 1){
                ...
                location.href=__URL(APPMAIN+"/goods/detail?goods_id="+id+"&bargain_id="+bargain_id);
            }else{
                if(ref_url != ''){
