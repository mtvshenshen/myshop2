...
function getArticleList(page_index){
	...
	if(is_load){
		is_load = false;
		api("System.Article.articleList",{ page_index : page_index, condition : { "nca.class_id" : $("#class_id").val()} },function (res) {
			var data = res.data;
			...
			if(data['data'].length >0 ){
				for(var i =0 ;i<data['data'].length;i++){
					...
					article_list_html+= '<img src="'+__IMG(item['image'])+'" class="lazy_load" alt="">';
				...
				article_list_html+= '<p class="no-article"><img src="' + WAPIMG + '/wap_nodata.png" height="60"><br>该分类下暂时没有文章！</p>';
			}
