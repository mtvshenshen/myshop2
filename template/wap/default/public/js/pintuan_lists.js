...
function GetDataList(page){
	...
	if(is_load) return;
	is_load = true;
	api("System.Pintuan.mySpellingOrderList",{ "page":page },function (res) {
		var data = res.data;
		...
		if(data['data'].length==0){
			...
			for (var i = 0; i < data['data'].length; i++) {
				...
				html += '<article onclick="location.href=\'' + __URL(APPMAIN+"/goods/detail?goods_id=" + curr.goods_id) + '\'">';
				...
				if (curr.status == 1) {
					...
					html += '</li>';
					html += '</ul>';
					html += '<button class="btn-order-info" onclick="location.href=\'' + __URL(APPMAIN+'/order/detail?orderId=' + curr.order_id) + '\'">查看订单详情</button>';
				}
				html += '</footer>';
				html += '</li>';
			}
			html += '</ul>';
			var user_img = WAPIMG+"/pintuan/member_default.png";
