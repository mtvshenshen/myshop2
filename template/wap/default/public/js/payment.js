...
var delivery_need_init = invoice_need_init = true; // 物流 发票 弹出页内容是否需要恢复初始状态
...
$(function(){
	...
	// 时间段选择
	$('.distribution-time-out .time-out-list span').click(function(event) {
		$(this).attr('class', 'selected ns-bg-color').siblings('span').attr('class', '');
	});
	// 时间选择
	$('.distribution-time-out .shipping-time-list li').click(function(event) {
		$(this).attr('class', 'selected ns-bg-color').siblings('li').attr('class', '');
	...
	// 优惠选择
	$('.coupon-list li').click(function(event) {
		if($(this).hasClass('selected')){
			$(this).removeClass('selected');
			$(this).children('i.iconfont').attr('class', 'iconfont iconcheckbox');
			...
			$(this).addClass('selected').siblings('li').removeClass('selected');
			$(this).siblings('li').children('i.iconfont').attr('class', 'iconfont iconcheckbox');
...
function Order(){
	...
	this.verify = function(){
		// 非虚拟商品
		if(data.is_virtual == 0){
			if(data.address_id == 0) { toast('请先选择收货地址'); return false; }
			// 如果用户选择商家配送的话 不考虑配送方式有没有开启
			if(data.pay_type != 4){
				if(data.shipping_info.shipping_type == undefined) { toast('商家未启用配送方式'); return false; }
			}
			if(data.shipping_info.shipping_type == 2 && data.shipping_info.pick_up_id == 0) { toast('请先选择自提点'); return false; }
		}else{
			if(data.user_telephone.search(regex.mobile) == -1) { toast('请输入正确的手机号'); return false; }
		}
		// 发票
		if($('#is-need-invoice').is(':checked')){
			if($('.invoice-wrap [name="invoice_title"]').val().search(/[\S]+/)) { toast('请填写发票抬头'); return false; }
			if($('.invoice-wrap [name="taxpayer_identification_number"]').val().search(/[\S]+/)) { toast('请输入纳税人识别号'); return false; }
		}
		return true;
	}	
}
