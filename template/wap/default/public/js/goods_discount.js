var lang_data = {};
$(function(){
	langApi(["goods_no_goods_you_want",
		"element",
		"days",
		"hours",
		"minutes",
		"second",
		"activity_over"], function (res) {
		lang_data = res;
	});
	...
	if(is_load){
		...
		api("System.Goods.goodsDiscountList",{ page_index : page, category_id : category_id },function (res) {
			...
			if(data['data'].length==0){
				$('.group-list').html('<p class="empty"><img src="'+WAPIMG+'/goods/wap_nodata.png"><br>Sorry！'+lang_data.goods_no_goods_you_want+'…</p>');
			}else{
				for(key in data['data']){
					...
					if(item.market_price > 0){
						list_html+= '<p class="buyer"><s>￥'+item.market_price+lang_data.element+'</s></p>';
					}
					list_html+= '</div>';
					list_html+= '</div></li>';
				}
	...
	$(".settime").each(function(i) {
		...
		if(end_date != undefined && end_date != ''){
			...
			if(sys_second>1){
				...
				self.html(day + lang_data.days + ( hour<10 ? "0" + hour : hour ) + lang_data.hours + (minute<10?"0"+minute:minute) + lang_data.minutes + (second<10?"0"+second:second) + lang_data.second);
			}
			var timer = setInterval(function(){
				if (sys_second > 1) {
					...
					self.html(lang_data.activity_over+"!");
					clearInterval(timer);
				}
			}, 1000);
		}
