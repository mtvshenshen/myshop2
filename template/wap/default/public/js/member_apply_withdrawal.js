            ...
            if (cash <= MaxCashAmount) {
                if (cash >= Minaamountcash) {
                    if (cash % parseInt(IntTimes) == 0) {
                        api('System.Member.addWithdrawApply', {"bank_account_id": bank_account_id, "cash": cash}, function(res) {
                            if (res.data >0) {
                                toast(lang_member_apply_withdrawal.member_submitted_for_review);
                                window.location.href = __URL(APPMAIN+"/member/balance?shop_id="+shop_id);
                            } else {
                                toast(res.message);
                            }
                        })
