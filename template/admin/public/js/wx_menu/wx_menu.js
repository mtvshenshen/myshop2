...
$(function() {
	...
	$(".jsSubMenu").live("click",function(){
		...
		$("#smallProgramUrltext").val("");
		$("#urltext").val("");//每次添加都清空链接
		$("#appid").val("");//每次添加都清空小程序appid	
		$("#pagepath").val("");//每次添加都清空小程序页面链接
...
//			showDialog("请先按要求编辑完当前操作！！！", 0);
	...
	$(".js-preview").live("click",function(){
		if($(this).attr("data-media-item-id")) window.open(__URL(APPMAIN + "/wchat/message?media_id=" + $(this).attr("data-media-item-id")));
	});
...
function subMenuClick(obj){
	...
	if(parseInt($(obj).find("a").attr("data-menu-type")) == 2 || parseInt($(obj).find("a").attr("data-menu-type")) == 3){
		...
		$("#url").hide();
		$("#small_program").hide();
	}else if(parseInt($(obj).find("a").attr("data-menu-type")) == 4){
		$(".js_small_program").addClass("selected").siblings().removeClass("selected");
		$("#edit").hide();
		$("#url").hide();
		$("#small_program").show();
	...
	var appid = $(obj).find("a").attr("data-appid");//小程序appid
	var pagepath = $(obj).find("a").attr("data-pagepath");//小程序跳转页路径
