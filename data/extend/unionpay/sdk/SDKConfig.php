...
class SDKConfig {
	...
	function __construct(){
		...
		//配置信息
		$config_service = new UnionPayConfig();
		$config = $config_service->getUnionpayConfig(0)['value'];
		
		//回调信息
		$notify_url = str_replace("/index.php", '', __URL__);
		$notify_url = str_replace("index.php", '', $notify_url);
		$notify_url = $notify_url . "/alipay.php";
		$backUrl = __URL(__URL__ . '/wap/Pay/backReceive');
		$frontUrl = __URL(__URL__ . '/wap/Pay/frontReceive');
		...
		$this->signCertPath = $config['certs_path'] . 'acp_test_sign.pfx';
		...
		$this->encryptCertPath = $config['certs_path'] . 'acp_test_enc.cer';
		$this->rootCertPath = $config['certs_path'] . 'acp_test_root.cer';
		$this->middleCertPath =   $config['certs_path'] . 'acp_test_middle.cer';
