...
class Upload extends BaseService
{
	...
	//上传路径列表
	//private $upload_directory = array();
	 ...
	 * @return unknown|number[]|unknown[][]|void
	 */
...
//				$filetype = $upload->getFileType($upload_data);//获取文件类型
	 ...
	 * @param number $size
	 * @return \data\service\Ambigous|number[]|unknown[][]|number[]|string[][]|unknown[][]|number[][]|boolean[][]|\data\service\Ambigous[][]|number[]
	 */
...
//				$filetype = $upload->getFileType($upload_data);//获取文件类型
				...
				$data = array(
					...
					"thumb_pic_path" => $thumb_data["thumb"]["thumb_name"],
					"thumb_pic_spec" => $thumb_data["thumb"]["width"] . "*" . $thumb_data["thumb"]["height"],
					"thumb_pic_size" => $thumb_data["thumb"]["width"],
					"create_time" => time()
				);
				...
				if ($album_data["pic_id"] == 0) {
					...
					$retval = $album->addPicture($file_name, $old_name, $album_id, $data["path"], $filesize,
						$file_info[0] . "," . $file_info[1], $thumb_data["big"]["thumb_name"],
						$thumb_data["big"]["size"],
						$thumb_data["big"]["width"] . "*" . $thumb_data["big"]["height"],
						$thumb_data["mid"]["thumb_name"], $thumb_data["mid"]["size"],
						$thumb_data["mid"]["width"] . "*" . $thumb_data["mid"]["height"],
						$thumb_data["small"]["thumb_name"], $thumb_data["small"]["size"],
						$thumb_data["small"]["width"] . "*" . $thumb_data["small"]["height"], $thumb_data["thumb"]["thumb_name"],
						$thumb_data["thumb"]["size"],
						$thumb_data["thumb"]["width"] . "*" . $thumb_data["thumb"]["height"], $this->instance_id, $this->upload_type,
						$file_res["domain"], $file_res["bucket"]);
				} else {
					$retval = $album->ModifyAlbumPicture($album_data['pic_id'], $data["path"], $filesize,
						$file_info[0] . "," . $file_info[1], $thumb_data["big"]["thumb_name"],
						$thumb_data["big"]["size"],
						$thumb_data["big"]["width"] . "*" . $thumb_data["big"]["height"],
						$thumb_data["mid"]["thumb_name"], $thumb_data["mid"]["size"],
	 ...
	 * @param unknown $file_upload_path
	 * @param number $size
	 * @return unknown|number[]|string[]|string[][]|number[][]|unknown[][]|number[]|NULL[]
	 */
	 ...
	 * @param unknown $file_upload_path
	 * @param number $size
	 * @return number[]|string[][]|number[]|string[]|unknown[][]|boolean[][]|string[][]|number[]|string[]
	 */
	...
	public function thumbCreate($upload_path, $file_name, $thumb_name, $width, $height)
	{
		try {
			...
			$image->save($upload_path . "/" . $thumb_name, null, 80);
			unset($image);
			//是否添加水印
