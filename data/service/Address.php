...
class Address extends BaseService
{
	...
	public function getCityId($city_name)
	{
		...
		if (empty($cache)) {
			...
			$cache = Cache::tag("address")->set("getCityId" . '_' . $city_name, $city_id);
			return $city_id;
		} else {
			return $cache;
		}
	...
	public function getDistrictId($district_name)
	{
		...
		if (empty($cache)) {
			...
			$cache = Cache::tag("address")->set("getDistrictId" . '_' . $district_name, $district_id);
			return $district_id;
		} else {
			return $cache;
		}
	...
	{
		Cache::tag("offpayarea")->set("getDistributionAreaInfo" . '_' . $shop_id, null);
