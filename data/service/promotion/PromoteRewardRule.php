...
class PromoteRewardRule extends BaseService
{
	...
	public function memberSign($uid, $shop_id)
	{
		...
		if ($config_info['sign_integral'] > 0) {
			//判断今天是否已经签到过
// 		    $count = $member_service->getIsMemberSign($uid, $shop_id);
// 			if ($count <= 0) {
				//查询 当前店铺签到赠送积分数量
...
// 			}
		...
		//签到送优惠券
		if ($config_info['sign_coupon'] > 0) {
			$info = $this->getRewardRuleDetail($shop_id);
			if ($info['sign_coupon'] != 0) {
			    $res = $member_service->memberGetCoupon($uid, $info['sign_coupon'], 2);
			}
		}
		return $res;
	}
	...
	public function memberShare($uid){
        ...
        $info = $this->getRewardRuleDetail(0);
        //判断今天是否已经分享过
	    ...
	    if ($config_info['share_integral'] > 0) {
	        if ($info['share_point'] != 0 && $count <= 0) {
    	        $res = $this->addMemberPointData(0, $uid, $info['share_point'], 6, '分享赠送积分');	            
	        }
	    }
	    
	    // 判断是否开启分享送优惠券
	    if ($config_info['share_coupon'] > 0) {
	        if ($info['share_coupon'] != 0 && $count <= 0) {
	            $res = $member->memberGetCoupon($uid, $info['share_coupon'], 2);
	        }
	    }
	...
	public function getSignInConfig(){
	    ...
	    $config_info = $config->getIntegralConfig(0);
	    $sign_in_info = [];
	    $sign_in_info['sign_integral'] = $config_info['sign_integral'];
	    $sign_in_info['sign_coupon'] = $config_info['sign_coupon'];
	    
	    $rule_info = $this->getRewardRuleDetail(0);
	    $integral_info = [];
	    $coupon_info = [];
	    if ($config_info['sign_integral'] > 0) {
	        //查询 当前店铺签到赠送积分数量
	        $integral_info["sign_point"] = $rule_info["sign_point"];
	    }
	    
	    //签到送优惠券
	    if ($config_info['sign_coupon'] > 0) {
	        if ($rule_info['sign_coupon'] != 0) {
	            $coupon_type = new NsCouponTypeModel();
	            $coupon_info = $coupon_type->getInfo(['coupon_type_id' => $coupon_type_id], '*');
	        }
	    }
