...
class Express extends BaseService
{
	 ...
	 * @param array $data
	 */
	public function addExpressCompany($param)
	{
		Cache::clear('express');
		$ns_express_company = new NsOrderExpressCompanyModel();
		...
		try {
			...
			$data = array(
			    ...
			    'express_logo' => $param['express_logo'],
			...
			$sid = $this->addExpressShipping($data['shop_id'], $data['company_name'], $co_id);
			$this->addExpressShippingItems($sid, $data['shop_id']);
	...
	{
		$cache = Cache::tag('express')->get('getDefaultShopExpressAddress' . $shop_id);
		if (!empty($cache)) {
			$data = $cache;
		} else {
			$shop_express_address = new NsShopExpressAddressModel();
			...
			Cache::tag('express')->set('getDefaultShopExpressAddress' . $shop_id, $data);
		}
	...
	{
		$cache = Cache::tag('express')->get('selectShopExpressAddressInfo' . $express_address_id);
		if (!empty($cache)) {
			return $cache;
		} else {
			$shop_express_address = new NsShopExpressAddressModel();
			...
			Cache::tag('express')->set('selectShopExpressAddressInfo' . $express_address_id, $retval);
			return $retval;
		}
	...
	{
		$cache = Cache::tag('express')->get('getExpressShippingItemsLibrary' . $shop_id);
		if (!empty($cache)) {
			return $cache;
		}
		...
		Cache::tag('express')->set('getExpressShippingItemsLibrary' . $shop_id, $item_list);
		return $item_list;
	}
