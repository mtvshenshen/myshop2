...
class QiNiu extends UploadParam{
    ...
    */
    public function setQiniuUplaod($filePath, $key){
            ...
            if ($err !== null) {
                return ["code"=>0,"path"=>"","domain"=>"", "bucket"=>"","message"=>"七牛云错误：".$err->getResponse()->error];
            } else {
                //返回图片的完整URL
                return ["code"=>1,"path"=>$this->QiniuUrl."/". $key,"domain"=>$this->QiniuUrl, "bucket"=>$this->Bucket,"message"=>"上传成功"];
            }
    }
