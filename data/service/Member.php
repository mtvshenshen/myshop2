...
class Member extends User
{
	 ...
	 */
	public function addMember($user_name, $password, $email, $sex, $status, $mobile, $member_level, $member_label)
	{
	...
	public function getShopNameByShopId($shop_id)
	{
		...
		$shop_info = $member_account->getInfo($condition, "shop_name");
		if (!empty($shop_info)) {
	 ...
	 * @param unknown $shop_id
	 * @param unknown $point
	 * @return string|Ambigous <number, string>|number
	 */
...
// 		);
	...
	{
		$member_level = new NsMemberLevelModel();
		$level_info = $member_level->getInfo([ "level" => $data["level"] ]);
		if (empty($level_info) || $level_info["level_id"] == $condition["level_id"]) {
...
// 		);
	 ...
	 * @param unknown $account_type_name
	 ...
	 * @param string $condition
	 * @param string $order
	 * @return Ambigous <multitype:number unknown , unknown>
	 */
	 ...
	 * @param unknown $transfer_type
	 * @param unknown $transfer_name
	 * @param unknown $transfer_money
	 * @param unknown $transfer_remark
	 * @param unknown $transfer_no
	 * @param unknown $transfer_account_no
...
public function getMemberCouponTypeList($shop_id, $uid)
	 ...
	 * @param unknown $member_level
	 */
	public function updateMemberByAdmin($uid, $user_name, $email, $sex, $status, $mobile, $nick_name, $member_level, $member_label)
	{
		...
		if ($retval < 0) {
			...
			//修改會員等級
			$res = $user_service->updateMemberLevelAction([ "level_id" => $member_level, "uid" => $uid ]);
			return $res;
		}
	}
	...
	/**
	 * 获取会员标签详情
	 * 创建时间：2019年1月22日15:25:19
	 * @param $id
	 * @return NsMemberLevelModel|null
	 */
	 ...
	 * 会员默认地址
	 * @param unknown $uid
	 * @return string|mixed|array|\think\db\false|PDOStatement|\think\Model|NULL
	 */
