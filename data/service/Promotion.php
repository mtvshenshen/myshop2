...
class Promotion extends BaseService
{
	...
	{
		$cache = Cache::tag('coupon')->get('getCouponTypeList' . json_encode([ $page_index, $page_size, $condition, $order ]));
		...
		Cache::tag('coupon')->set('getCouponTypeList' . json_encode([ $page_index, $page_size, $condition, $order ]), $coupon_type_list);
	...
	{
		$cache = Cache::tag('coupon')->get('getTypeCouponList_' . $coupon_type_id . '_' . $get_type . '_' . $use_type);
		if (!empty($cache)) return $cache;
		...
		Cache::tag('coupon')->set('getTypeCouponList_' . $coupon_type_id . '_' . $get_type . '_' . $use_type, $list['data']);
		return $list['data'];
		// TODO Auto-generated method stub
	}
	 ...
	 * @param string $condition
	 * @param string $order
	 * @return Ambigous <string, multitype:number unknown >
	 */
	 ...
	 * @return multitype:number string |string|number
	 */
	...
	public function updatePromotionMansong($mansong_id, $mansong_name, $start_time, $end_time, $shop_id, $remark, $type, $range_type, $rule, $goods_id_array)
	{
		...
		try {
			...
			if ($range_type == 0 && !empty($goods_id_array)) {
				...
				$goods_promotion_model = new NsGoodsPromotionModel();
				$goods_promotion_model->destroy([ 'promotion_id' => $mansong_id ]);
				$data_goods_promotion = [
					'goods_id' => 0,
					'label' => '满',
					'remark' => '',
	 ...
	 * @param unknown $remark
	 * @param unknown $goods_id_array
	 * @param unknown $decimal_reservation_number
	 * @return string|unknown
	 */
	...
	public function getPromotionDiscountDetail($discount_id)
	{
		...
		if (!empty($promotion_goods_list)) {
			foreach ($promotion_goods_list as $k => $v) {
				...
				$goods_info = $goods->getInfo([
					'goods_id' => $v['goods_id']
				], 'price, stock, picture, goods_name');
				$picture = new AlbumPictureModel();
				$pic_info = array();
				$pic_info['pic_cover'] = '';
				if (!empty($v['goods_picture'])) {
				...
				$v['price'] = $goods_info['price'];
				$v['stock'] = $goods_info['stock'];
				$decimal_reservation_number = $promotion_detail['decimal_reservation_number'] == -1 ? 2 : $promotion_detail['decimal_reservation_number'];
				$v['promotion_price'] = sprintf("%.2f", round(($goods_info['price'] * $v['discount'] / 10), $decimal_reservation_number));
			}
		}
	 ...
	 * @param unknown $goods_picture
	 * @param unknown $type
	 * @param unknown $type_name
	 * @param unknown $relate_id
	 * @param unknown $remark
		...
		try {
			...
			if ($rule_id > 0) {
				...
				if ($game_rule_info['type'] == 1) {
					...
					$sss = $member_account_service->addMemberAccountData($shop_id, 1, $uid, 0, $game_rule_info['points'], 11, $game_id, "参与【" . $game_info['name'] . "】营销游戏，获得【" . $game_rule_info['rule_name'] . "】作为奖励，赠送" . $game_rule_info['points'] . "积分");
	...
	public function userAchieveGift($uid, $gift_records_id, $receiver_mobile, $receiver_province, $receiver_city, $receiver_district, $receiver_address, $receiver_zip, $receiver_name, $buyer_message, $fixed_telephone = "")
	{
		...
		try {
			...
			$data_order = array(
				'order_type' => 1,
				'order_no' => $order_create->createOrderNo($gift_records_info['shop_id']),
				'out_trade_no' => '',
				'payment_type' => 1,
				'shipping_type' => 1,
				'order_from' => 1,
			...
			$res_order_goods = $order_action->addOrderGiftGoods($order_id, $sku_info['sku_id'] . ':1');
			// 订单项
...
//                      'end_time' => getTimeTurnTimeStamp($end_time),
	 ...
	 * @param unknown $desc
	 * @param unknown $picture_img
	 * @param unknown $scroll_img
	 * @param unknown $background_img
	 * @param unknown $background_color
	 * @param unknown $start_time
	 * @param unknown $end_time
	 * @param unknown $content
	 * @param unknown $goods_id_array
	 * @param unknown $is_head
	 * @param unknown $is_foot
	 * @param unknown $pc_topic_template
	 * @param unknown $wap_topic_template
	 */
...
//      dump($promotion_detail);die;
	...
	{
		$result = [];
		$list = $this->getPromotionDiscountList(1, 1, [ 'status' => 1 ], 'discount_id desc', 'discount_id');
		if (!empty($list['data'])) {
			$result = $this->getPromotionDiscountDetail($list['data'][0]['discount_id']);
		}
		return $result;
	}
	 ...
	 */
	public function getGoodsPromotion($condition, $field = "*", $order = "end_time desc")
	{
		$goods_promotion_model = new NsGoodsPromotionModel();
