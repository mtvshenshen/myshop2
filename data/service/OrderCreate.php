...
class OrderCreate extends OrderService
{
    ...
    public function orderCreate($data)
    {
        ...
        try {
            ...
            foreach($data["goods_sku_array"] as $data_k => $data_v){
                ...
                //库存判断
                if ($data_v['goods_sku_info']['stock'] < $data_v["num"] || $data_v["num"] <= 0) {
                    $this->order->rollback();
                ...
                $goods_calculate->subGoodsStock($data_v['goods_id'], $data_v['sku_id'], $data_v["num"], '');
     ...
     * @param unknown $uid
     */
    private function getGoodsSkuMemberPrice($sku_info, $uid)
    {
        ...
        if(!empty($goods_member_discount_detail["discount"])){
            return $goods_member_discount = $goods_member_discount_detail["discount"] / 100;
        }else{
            return 0;
        }
     ...
     * @return number|\data\model\unknown
     */
...
//                 }
     ...
     * 获取订单所需 站点信息
     */
    public function getOrderShopInfo(){
        $data = [];
        ...
        $member_address = $member_service->getMemberDefaultAddress($data["buyer_id"]);
                ...
                if (! empty($list_quan[0])) {
                    ...
                    if (! empty($mansong_list)) {
                        ...
                        foreach ($mansong_list as $k => $v) {
                            ...
                            foreach ($data["goods_sku_array"] as $k => $v) {
                                //判断是否存在与活动商品数据中
                                if (in_array($v["goods_id"], $mansong_goods_id_array) !== false) {
                                    
                                    $new_sku_list_array[] = $v;
                                    $new_sku_list_price += $v["total_money"];
                                }
                            }
    ...
    public function getOrderGiftList($data){
        ...
        if(!empty($data["gift_id_array"])){
            foreach ($data["gift_id_array"] as $k => $v) {
                ...
                $sku_id = $goods_sku_model->getInfo(['goods_id' => $goods_id['goods_id']], 'sku_id');
    ...
    public function orderCalculate($data){
        ...
        $shop_data = $this->getOrderShopInfo();//站点信息
        ...
        //砍价活动
        if($order_data["promotion_type"] == 3){
            $shipping_money = 0;//砍价订单默认免邮
        ...
        $order_money = $order_data["total_money"] + $shipping_money;
...
//         }
     ...
     * @param unknown $data
     */
    public function getExplessCompany($data){
