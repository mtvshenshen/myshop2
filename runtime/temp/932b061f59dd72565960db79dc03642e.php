...
<div class="options-btn">
	...
	<button class="btn-common" onclick="location.href='<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/editAppUpgrade'); ?>';">添加App版本</button>
</div>
...
function LoadingInfo(page_index) {
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/appUpgradeList'); ?>",
			...
			if (data["data"].length > 0) {
				...
				for (var i = 0; i < data["data"].length; i++) {
					...
					html += '<tr align="center">';
						...
						html += '<td align="left"><a href="'+ __IMG(data["data"][i]["download_address"]) +'">'+ __IMG(data["data"][i]["download_address"]) +'</a></td>';
						html += '<td>'+ timeStampTurnTime(data['data'][i]['create_time']) +'</td>';
						html += '<td><a href="'+__URL('http://127.0.0.1:8080/index.php/admin/config/editAppUpgrade?id='+ data['data'][i]['id'])+'">修改</a><a style="cursor: pointer;" onclick="deleteLink('+data['data'][i]['id']+')">删除</a></td> ';
					html += '</tr>';
		...
		buttons: {
			"确定": function() {
				$(this).dialog('close');
					$.ajax({
					type:"post",
					url:"<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/deleteAppUpgrade'); ?>",
					data:{ 'id':id.toString() },
					success:function (data) {
						if(data['code'] > 0){
