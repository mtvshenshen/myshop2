...
<div class="options-btn">
	<button class="btn-common btn-small" onclick="location.href='<?php echo __URL('http://127.0.0.1:8080/index.php/admin/member/editmemberlabel'); ?>';">添加标签</button>
</div>
...
function LoadingInfo(page_index) {
	$.ajax({
		type : "post",
		url : __URL("http://127.0.0.1:8080/index.php/admin/member/memberlabellist"),
		...
		success : function(data) {
			if (data["data"].length > 0) {
				...
				for (var i = 0; i < data["data"].length; i++) {
					...
					html += '<td><a href="'+__URL('http://127.0.0.1:8080/index.php/admin/member/editmemberlabel?id='+ data['data'][i]['id'])+'">修改</a>&nbsp;&nbsp;&nbsp;<a href="javascript:deleteMemberLabel('+data['data'][i]['id']+');">删除</a></td>';
					html += '</tr>';
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				$.ajax({
					type : "post",
					url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/member/deletememberlabel'); ?>",
					data : { 'id' : id },
					success : function(data) {
						if (data['code'] > 0) {
