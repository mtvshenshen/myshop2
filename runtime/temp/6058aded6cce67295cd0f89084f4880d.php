...
<div class="options-btn">
	<button class="btn-common" onclick="location.href='<?php echo __URL('http://127.0.0.1:8080/index.php/admin/extend/addhooks'); ?>';">添加钩子</button>
</div>
...
function LoadingInfo(page_index) {
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/extend/hookslist'); ?>",
		...
		success : function(data) {
			if (data['data'].length > 0) {
				...
				for (var i = 0; i < data['data'].length; i++) {
					...
					html += '<td align="center"><a href="'+__URL('http://127.0.0.1:8080/index.php/admin/extend/updatehooks?id='+data['data'][i]['id'])+'" style="margin-left:0;">修改</a></td>';
					html += '</tr>';
	...
	$("#dialog").dialog({
		buttons: {
			"确定": function() {
				...
				$.ajax({
					type : "post",
					url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/extend/deleteHooks'); ?>",
					data : { "id" : id },
					success : function(data) {
						if(data['code'] > 0){
							showTip(data["message"],'success');
							location.href = "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/extend/hookslist'); ?>";
						}else{
