...
<div class="options-btn">
	...
	<button class="btn-common" onclick="location.href='<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/adddocument'); ?>';">添加帮助内容</button>
</div>
...
function LoadingInfo(page_index) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/helpdocument'); ?>",
		...
		success : function(data) {
			if (data["data"].length > 0) {
				...
				for (var i = 0; i < data["data"].length; i++) {
					...
					html += '<td><a href="'+__URL('http://127.0.0.1:8080/index.php/admin/config/updatedocument?id='+data['data'][i]['id'])+'" >修改</a>&nbsp;&nbsp;<a href="javascript:help_delete('+data['data'][i]['id']+')">删除</a>&nbsp;&nbsp;<a href="'+__URL('http://127.0.0.1:8080/index.php/help/index?id='+ data['data'][i]['id']+'&class_id='+data["data"][i]["class_id"])+'" target="_blank">预览</a></td>';
					html += '</tr>';
...
function updateDocument(id,index,class_id){
	...
	$.ajax({
		type:"post",
		url:"<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/updateHelpContentTitleAndSort'); ?>",
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				...
				$.ajax({
					type : "post",
					url: "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/titledelete'); ?>",
					data: { "id": id.toString()},
					success : function(data) {
						if(data["code"] > 0){
							LoadingInfo(getCurrentIndex(id,'.table-class tbody'));
