...
.add-child{
	padding-left: 15px;
	background: url(/template/admin/public/images/add-child.png) no-repeat;
...
<div class="options-btn">
	<button class="btn-common btn-small" onclick="location.href='<?php echo __URL('http://127.0.0.1:8080/index.php/admin/goods/addgoodscategory'); ?>';">添加商品分类</button>
</div>
	...
	<tbody>
		...
		<tr class="pid_0" style="height: 30px;" data-category-id="<?php echo $v1['category_id']; ?>" data-pid="<?php echo $v1['pid']; ?>" data-level="<?php echo $v1['level']; ?>">
			...
			<td style="text-align: center;">
				<a href="<?php echo __URL('http://127.0.0.1:8080/index.php/admin/goods/updategoodscategory','category_id='.$v1['category_id']); ?>">修改</a>
			...
			<tr class="pid_<?php echo $v1['category_id']; ?>" style="height: 30px;display:none;" data-category-id="<?php echo $v2['category_id']; ?>" data-pid="<?php echo $v2['pid']; ?>" data-level="<?php echo $v2['level']; ?>">
				...
				<td style="text-align: center;">
					<a href="<?php echo __URL('http://127.0.0.1:8080/index.php/admin/goods/updategoodscategory','category_id='.$v2['category_id']); ?>">修改</a>
				...
				<tr class="pid_<?php echo $v2['category_id']; ?> pid_<?php echo $v1['category_id']; ?>" style="height: 30px;display:none;" data-category-id="<?php echo $v3['category_id']; ?>" data-pid="<?php echo $v3['pid']; ?>" data-level="<?php echo $v3['level']; ?>">
					...
					<td style="text-align: center;">
						<a href="<?php echo __URL('http://127.0.0.1:8080/index.php/admin/goods/updategoodscategory','category_id='.$v3['category_id']); ?>">修改</a>
...
<?php else: ?>
	<div class="tip-box">当前系统还没有商品分类，<a href="<?php echo __URL('http://127.0.0.1:8080/index.php/admin/goods/addgoodscategory'); ?>">马上添加</a></div>
<?php endif; ?>
...
$(".table-class tr[data-category-id] input").live("change",function(){
	...
	if(fieldvalue == ''){
		...
		$.ajax({
			type:"post",
			url:"<?php echo __URL('http://127.0.0.1:8080/index.php/admin/goods/modifygoodscategoryfield'); ?>",
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				...
				$.ajax({
					type : "post",
					url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/goods/deletegoodscategory'); ?>",
					data : {
					...
					success : function(data) {
						if(data['code'] > 0){
							...
							location.href = "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/goods/goodscategorylist'); ?>";
						}else{
...
function getCookieGoodsCategory(category_id){
	...
	$.ajax({
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/goods/getquickgoods'); ?>",
		type : "post",
		async : false,
		success : function(res) {
...
$("#saveNewAddedCategory button").live('click', function(event) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/goods/batchAddgoodscategory'); ?>",
...
function save(){
	...
	if(verify( category_name,short_name, sort)){
		...
		$.ajax({
			type : "post",
			url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/goods/addgoodscategory'); ?>",
			data : {
