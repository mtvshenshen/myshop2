...
<table class="mytable">
	<tr>
		<th width="20%" style="text-align: left;">
			<button class="btn-common btn-small" onclick="location.href='<?php echo __URL('http://127.0.0.1:8080/index.php/admin/block/edit'); ?>';" style="margin:0 5px 0 0 !important;">添加</button>
...
function LoadingInfo(page_index) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/block/goodsfloorblock'); ?>",
		data : {},
		success : function(data) {
			if (data["data"].length > 0) {
				...
				for (var i = 0; i < data["data"].length; i++) {
					...
					html += '<td>';
					html += '<a href="'+__URL('http://127.0.0.1:8080/index.php/admin/block/edit?block_id=' + data["data"][i]["id"])+'">编辑</a>&nbsp;';	
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				$.ajax({
					type:"post",
					url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/block/deleteFloor'); ?>",
					data : { "id" : id.toString() },
					dataType : "json",
					success : function(data) {
						if (data["code"] > 0) {
...
function load(){
	$('.input-common-sort').change(function(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo __URL('http://127.0.0.1:8080/index.php/admin/block/modifyFloorSort'); ?>",
