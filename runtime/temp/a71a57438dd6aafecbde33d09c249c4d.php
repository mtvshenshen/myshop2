...
<div id="pictureIndex" class="ncsc-picture-folder">
	<div class="ncsc-album">
		...
		<?php endif; if($info['is_check'] == 1): if(file_exists("$info[template_url]")): ?>
				...
				<?php else: ?>
				<img src="/public/static/images/qrcode_bg/template_bg_img.png" style="border:1px solid #00C0FF;"/>
				<?php endif; else: if(file_exists("$info[template_url]")): ?>
				<img src="<?php echo __IMG($info['template_url']); ?>" style="border:1px solid #eee;"/>
				<?php else: ?>
				<img src="/public/static/images/qrcode_bg/template_bg_img.png" style="border:1px solid #eee;"/>
				<?php endif; endif; ?>
				<div class="check" >
				...
				<div>
					<?php if($info['is_check'] == 1): ?> 
						<img src="/template/admin/public/images/check_qrcode1.png" class="qrcode_img" style="position:absolute;top:0px;left:1px;"/>
					<?php else: ?> 
						<img src="/template/admin/public/images/check_qrcode1.png" class="qrcode_img" style="position:absolute;top:0px;leftt:1px;display:none;"/>
					<?php endif; ?>
				</div>
			</li>
		<?php endforeach; endif; else: echo "" ;endif; ?>
...
function save(event){
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/wchat/qrcode'); ?>",
		data : {
		...
		success : function(data) {
			if(data['code'] > 0){
				...
				location.href = "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/wchat/weixinqrcodetemplate?type='.$type); ?>";
			}else{
...
function modifyWeixinQrcodeTemplateValid(id){
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/wchat/modifyweixinqrcodetemplatevalid'); ?>",
...
function deleteWeixinQrcodeTemplateValid(id){
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/wchat/deleteweixinqrcodetemplatevalid'); ?>",
		data : { 'id' : id },
		success : function(data) {
			if(data['code'] > 0){
				showTip(data['message'],"success");
...
function load_url(){
	window.location.href="<?php echo __URL('http://127.0.0.1:8080/index.php/admin/wchat/qrcode?type='.$type); ?>";
}

function modifyWeixinQrcode(id){
	window.location.href=__URL('http://127.0.0.1:8080/index.php/admin/wchat/qrcode?id=' + id+'&type=<?php echo $type; ?>');
}
</script>
