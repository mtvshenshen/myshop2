...
<script type="text/javascript" charset="utf-8" src="/template/admin/public/js/ueditor/ueditor.all.common.js"></script>
<style>
	#editor{width: 100%;}
</style>
...
<div class="set-style">
	<dl>
		...
		<dd>
			<input id="title" type="text" class="input-common" value="<?php echo $info['title']; ?>"/>
	...
	<dl>
		...
		<dd>
			<script id="editor" type="text/plain"><?php echo $info['content']; ?></script>
...
function save(){
	...
	if(verification(title,content)){
		...
		if(!flag){
			...
			$.ajax({
				type : "post",
				url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/member/registrationAgreement'); ?>",
				data : {
					'title' : title,
					'content' : content,
				},
				success : function(data){
					if(data['code'] > 0){
						...
						location.href = "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/member/registrationAgreement'); ?>";
					}else{
						showTip("发布失败","error");
					}
					flag = false;
