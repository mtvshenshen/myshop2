...
<script src="/template/admin/public/js/layui/layui.js"></script>
<link rel="stylesheet" type="text/css" href="/template/admin/public/js/layui/css/layui.css" />
<link rel="stylesheet" type="text/css" href="/template/admin/public/css/block.css" />
<div class="set-style">
	<input type="hidden" id="hidden_block_id">
		<dl>
			<dt><span class="color-red"></span>楼层名称</dt>
			<dd>
				<p><input name="" id="floor_name" value="<?php echo $block_info['name']; ?>" type="text" style="width:300px;" class="input-common"></p>
				<p class="error"></p>
			</dd>
		</dl>
		<dl>
			...
			<dd>
				<div class="controls">
					<p>
						<input id="is_use" type="checkbox"  class="checkbox" <?php if($block_info['is_use'] == 1): ?>checked<?php endif; ?>/>
					</p>
				</div>
			</dd>
		</dl>
			...
			<dd>
				<div class="controls">
					<select name="block_template" id="class_id" class="select-common" onchange="blockTemplate()">
					   ...
					   <?php if(is_array($block_template_list) || $block_template_list instanceof \think\Collection || $block_template_list instanceof \think\Paginator): if( count($block_template_list)==0 ) : echo "" ;else: foreach($block_template_list as $k=>$block): ?>
						<option value="<?php echo $block; ?>"  <?php if($block==$block_info['block_template']): ?>selected="selected"<?php elseif($k==0): ?>selected="selected"}<?php endif; ?>><?php echo $block; ?></option>
					   <?php endforeach; endif; else: echo "" ;endif; ?>
					</select>
				</div>
			</dd>
	...
	<section class="block-main" <?php if($block_html): ?>style="display:block;"<?php endif; ?>><?php echo $block_html; ?></section>
...
<div class="layui-form">
    ...
    <div class="layui-form-item">
        <div class="layui-input-block">
            ...
            <input type="hidden" name="pc_template" value="<?php echo !empty($pc_template['pc_template'])?$pc_template['pc_template'] :$pc_template; ?>" />
...
<script>
    ...
    <?php if($block_info['value']!=''): ?>
    var session_data = eval('(<?php echo $block_info['value']; ?>)');
    <?php endif; ?>
    ...
    function save() {
	     ...
	     $.ajax({
	         type : "post",
	         url : 'http://127.0.0.1:8080/index.php/admin/block/edit',
	         dataType : "json",
	         data : data,
	         success : function (res) {
	             console.log(res);
	             if(res.code > 0){
	                 location.href = 'http://127.0.0.1:8080/index.php/admin/block/goodsFloorBlock';
	             }else{
	                 flag = true;
	             }
...
<script src="/template/admin/public/js/block_template.js"></script>
