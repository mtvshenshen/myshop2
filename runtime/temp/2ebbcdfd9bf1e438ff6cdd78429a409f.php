...
<div class="options-btn">
	<button class="btn-common btn-small" onclick="location.href='<?php echo __URL('http://127.0.0.1:8080/index.php/admin/member/addmemberlevel'); ?>';">添加等级</button>
</div>
<div class="mytable select border-line">
	...
	<div>
		<div class="nav-config-block">
			...
			<div class="nav-config-content">
				<label for="level_type1" class="radio inline normal"><i class="radio-common <?php if($config_info['type'] == 1): ?>selected<?php endif; ?>"><input type="radio" value="1" name="level_type" id="level_type1" <?php if($config_info['type'] == 1): ?>checked="checked"<?php endif; ?>/></i>
				累计积分</label>
				<label for="level_type2" class="radio inline normal"><i class="radio-common <?php if($config_info['type'] == 2): ?>selected<?php endif; ?>"><input type="radio" value="2" name="level_type" id="level_type2" <?php if($config_info['type'] == 2): ?>checked="checked"<?php endif; ?>/></i>
				累计消费</label>
				<label for="level_type3" class="radio inline normal"><i class="radio-common <?php if($config_info['type'] == 3): ?>selected<?php endif; ?>"><input type="radio" value="3" name="level_type" id="level_type3" <?php if($config_info['type'] == 3): ?>checked="checked"<?php endif; ?>/></i>
				购买次数</label>
				<!-- <label for="level_type4" class="radio inline normal"><i class="radio-common <?php if($config_info['type'] == 4): ?>selected<?php endif; ?>"><input type="radio" value="4" name="level_type" id="level_type4" <?php if($config_info['type'] == 4): ?>checked="checked"<?php endif; ?>/></i>
				购买商品</label> -->
			</div>
		</div>
	</div>
	<div>
		<div class="nav-config-block">
			...
			<div class="nav-config-content">会员等级按照升级规则的标准进行升级，请慎重选择升级规则。</div>
		</div>
	</div>
</div>
...
$(function(){
	$("input[name='level_type']").change(function(){
		...
		$.ajax({
			type : "post",
			url : __URL("http://127.0.0.1:8080/index.php/admin/member/setMemberLevelConfig"),
			data : { "type" : val},
...
function LoadingInfo(page_index) {
	...
	$.ajax({
		type : "post",
		url : __URL("http://127.0.0.1:8080/index.php/admin/member/memberlevellist"),
		data : { 
		...
		success : function(data) {
			if (data["data"].length > 0) {
				...
				for (var i = 0; i < data["data"].length; i++) {
					...
					html += '<td align="left"><div class="<?php if($config_info['type'] == 1): ?>dis-block<?php else: ?>dis-none<?php endif; ?>">累计积分满' + data['data'][i]['min_integral'] +'分</div>';

					html += '<div class="<?php if($config_info['type'] == 2): ?>dis-block<?php else: ?>dis-none<?php endif; ?>">消费额满' + data['data'][i]['quota'] +'元'+'</div>';

					html += '<div class="<?php if($config_info['type'] == 3): ?>dis-block<?php else: ?>dis-none<?php endif; ?>">购买量满' + data['data'][i]['min_integral'] +'笔</div></td>';
					
					
					html += '<td align="left">' + data['data'][i]['goods_discount'] * 100 + '<span class="row-50"></span></td>';
					html += '<td align="left">'+ data['data'][i]['level'] +'</td>';
					if(data['data'][i]['is_default'] == 1){
						html += '<td> <a href="'+__URL('http://127.0.0.1:8080/index.php/admin/member/updatememberlevel?level_id='+ data['data'][i]['level_id'])+'">修改</a></td>';
					}else{
						html += '<td><a href="'+__URL('http://127.0.0.1:8080/index.php/admin/member/updatememberlevel?level_id='+ data['data'][i]['level_id'])+'">修改</a>&nbsp;&nbsp;&nbsp;<a href="javascript:deleteMember('+data['data'][i]['level_id']+');">删除</a></td>';
					}
					html += '</tr>';
...
function deleteMember(level_id){
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/member/deleteMemberLevel'); ?>",
		data : { 'level_id' : level_id },
		success : function(data) {
			if (data['code'] > 0) {
				showTip(data['message'],'success');
				location.href=__URL("http://127.0.0.1:8080/index.php/admin/member/memberlevellist");
			} else {
				showTip(data['message'],'error');
			}
		}
