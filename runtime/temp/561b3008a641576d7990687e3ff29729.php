...
<table class="mytable">
	<tr>
		<td width="50%" style="text-align: left;">
			...
			<button class="btn-common" onclick="location.href='<?php echo __URL('http://127.0.0.1:8080/index.php/admin/cms/addarticle'); ?>';" style="margin:0 5px 0 0 !important;">添加文章</button>
		</td>
		<td width="20%">
...
$('.num').live("change",function(){
	...
	$.ajax({
		type:"post",
		url:"<?php echo __URL('http://127.0.0.1:8080/index.php/admin/cms/modifyarticlefield'); ?>",
...
function LoadingInfo(page_index) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/cms/articlelist'); ?>",
		...
		success : function(data) {
			if (data["data"].length > 0) {
				...
				for (var i = 0; i < data["data"].length; i++) {
					...
					html += '<td><a target="_blank" href="'+__URL('http://127.0.0.1:8080/index.php/article/detail?article_id='+ data['data'][i]['article_id']+'&class_id='+data["data"][i]["class_id"])+'">预览</a><a href="'+__URL('http://127.0.0.1:8080/index.php/admin/cms/updatearticle?id='+ data['data'][i]['article_id'])+'">修改</a><a style="cursor: pointer;" onclick="deleteArticle('+data['data'][i]['article_id']+')">删除</a></td> ';
					html += '</tr>';
		...
		buttons: {
			"确定": function() {
				...
				if(!flag){
					...
					$.ajax({
						type : "post",
						url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/cms/deletearticle'); ?>",
