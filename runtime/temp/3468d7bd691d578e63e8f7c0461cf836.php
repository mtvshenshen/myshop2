...
<div class="set-style">
	<dl>
		<dt>最低提现金额：</dt>
		<dd>
			<input id="cash_min" type="number" min="0.00" value="<?php echo $list['value']['withdraw_cash_min']; ?>" class="input-common harf"/><em class="unit">元</em>
	...
	<dl>
		<dt>提现倍数：</dt>
		<dd>
			<input id="multiple" type="number" min="0.00" value="<?php echo $list['value']['withdraw_multiple']; ?>" class="input-common harf" /><em class="unit">倍</em>
	...
	<dl>
		<dt>提现提示信息：</dt>
		<dd>
			<textarea rows="2" id="message" class="textarea-common" maxlength="50"><?php echo $list['value']['withdraw_message']; ?></textarea>
	...
	<dl>
		<dt>提现账户：</dt>
		<dd>
			<?php if(is_array($list['value']['withdraw_account']) || $list['value']['withdraw_account'] instanceof \think\Collection || $list['value']['withdraw_account'] instanceof \think\Paginator): if( count($list['value']['withdraw_account'])==0 ) : echo "" ;else: foreach($list['value']['withdraw_account'] as $key=>$vo): ?>
			<i class="checkbox-common <?php if($vo['is_checked']==1): ?>selected<?php endif; ?>"><input id="<?php echo $vo['id']; ?>" type="checkbox" value="<?php echo $vo['value']; ?>" name="withdraw_account" <?php if($vo['is_checked']==1): ?>checked="checked"<?php endif; ?> /></i>
			<label class="common-label" for="<?php echo $vo['id']; ?>"><?php echo $vo['name']; ?></label>
			<?php endforeach; endif; else: echo "" ;endif; ?>
		</dd>
	</dl>
	<dl>
		...
		<dd>
			<div class="controls">
				<i class="radio-common"><input type="radio" name="enable" value="1" checked="checked" id="enableYes"></i><label class="common-label" for="enableYes">启用</label> 
				<i class="radio-common"><input type="radio" name="enable" value="0" id="enableNo"></i><label class="common-label" for="enableNo">禁用</label>
	...
	<dl>
		<dt></dt>
		<dd><button class="btn-common btn-big" onclick="setMemberwithdrawAjax();">保存</button></dd>
	</dl>
</div>
<script>
...
function setMemberwithdrawAjax() {
	...
	if(validation){
		$.ajax({
			type : "post",
			url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/memberwithdrawsetting'); ?>",
			data : {
				'cash_min' : cash_min,
				'multiple' : multiple,
				'poundage' : poundage,
...
$(function(){
	...
	if(parseInt(is_open) == 0){
		...
		$("#enableNo").parent().addClass("selected");
	}else{
		$("#enableYes").prop("checked","checked");
		$("#enableYes").parent().addClass("selected");
	}
})
</script>
