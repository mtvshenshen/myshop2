...
<div class="options-btn">
	...
	<button class="btn-common" onclick="location.href='<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/addRoutingRules'); ?>';">添加路由规则</button>
</div>
...
function LoadingInfo(page_index) {
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/customPseudoStaticRule'); ?>",
		...
		success : function(data) {
			if(data["data"].length > 0){
				...
				for(var i = 0; i < data["data"].length; i++){
					...
					if(item["is_open"] == 0){
						html += '<td><img src="/template/admin/public/images/un_checked.png" alt="" width="15"/></td>';
					}else{
						html += '<td><img src="/template/admin/public/images/checked.png" alt="" width="15"/></td>';
					}
					if(item["is_system"] == 0){
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				...
				$.ajax({
					type : "post",
					url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/delete_url_route'); ?>",
					data : { "routeid" : routeid.toString() },
					success : function(data) {
						if (data["code"] > 0) {
							showTip(data["message"],'success');
							location.href = "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/customPseudoStaticRule'); ?>";
						}else{
