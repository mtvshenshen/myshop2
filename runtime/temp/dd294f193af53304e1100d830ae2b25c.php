...
<div class="options-btn">
	...
	<button class="btn-common" onclick="location.href='<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/addHomeNotice'); ?>';">添加首页公告</button>
</div>
...
function LoadingInfo(page_index) {
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/userNotice'); ?>",
		...
		success : function(data) {
			if (data["data"].length > 0) {
				...
				for (var i = 0; i < data["data"].length; i++) {
					...
					html += '<td><a href="'+__URL('http://127.0.0.1:8080/index.php/admin/config/updateHomeNotice?id='+ data['data'][i]['id'])+'">修改</a><a style="cursor: pointer;" onclick="deleteNotice('+data['data'][i]['id']+')">删除</a></td> ';
					html += '</tr>';
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				...
				$.ajax({
					type : "post",
					url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/deleteNotice'); ?>",
					data : { "id" : id.toString() },
					success : function(data) {
						if (data["code"] > 0) {
...
function load(){
	$('.input-common-sort').change(function(){
		...
		$.ajax({
			type:"post",
			url:"<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/modifyNoticeSort'); ?>",
