...
<!-- 修改会员密码 -->
<div class="modal fade hide" id="modify_fen" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">

	<div class="modal-dialog">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
				<h3 id="myModalLabel">分成比例</h3>
			</div>
			<div class="modal-body">
				<div class="modal-infp-style">
					<table class="modal-tab">
						<tr>
							<td style="width:20%">比例（填写整数，按百分比计算，如填写50，会按50%计算）</td>
							<td colspan='3'><input type="text" id="modify_fens" class="input-common"/></td>
						</tr>
					</table>
				</div>
			</div>
			...
			<div class="modal-footer">
				...
				<button class="btn-common btn-big" onclick="modifyfen()">保存</button>
				<button class="btn-common-cancle btn-big" data-dismiss="modal">关闭</button>
			</div>
		</div>
	</div>
...
function LoadingInfo(page_index) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/member/shopAgree'); ?>",
		data : {
		...
		success : function(data) {
			if (data["data"].length > 0) {
				...
				for (var i = 0; i < data["data"].length; i++) {
					...
					html += '<td>' + data["data"][i]["user_shop_fen"] +'</td>';
					...
					if(data["data"][i]["user_shop"] == 0){
						...
						html += '<a onclick="modify_fen('+ data["data"][i]["uid"]+')">利润分成设置</a>';						
...
//修改会员弹出
function modify_fen(uid){
	$("#modify_fen").modal("show");
...
function modifyfen(){
	...
	var password = $("#modify_fens").val();

	if (password == null || !Number.isInteger(+password)) {
		showTip('比例必须为整数！','warning');
		return false;
	}
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/member/updatefen'); ?>",
		data : {
			'uid':uid,
			'user_fen' :password
		},
		success : function(data) {
				...
				LoadingInfo(getCurrentIndex(uid,'#productTbody'));
				$("#modify_fen").modal("hide");window.location.reload(); 
