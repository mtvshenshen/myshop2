...
<table class="mytable">
	<tr>
		<td style="text-align: left;"><button class="btn-common" onclick="location.href='<?php echo __URL('http://127.0.0.1:8080/index.php/admin/auth/adduser'); ?>';" style="margin:0 !important;">添加用户</button></td>
		<td>
...
function LoadingInfo(page_index) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/auth/userlist'); ?>",
			...
			if (data["data"].length > 0) {
				...
				for (var i = 0; i < data["data"].length; i++) {
					...
					if(data["data"][i]["user_headimg"] == ""){
						html += '<img src="/public/static/images/default_user_portrait.gif" class="head-portrait" />';
					}else{
					...
					if(data['data'][i]['is_admin'] == 1){
						...
						html += '<td>';
							html += '<a href="'+__URL('http://127.0.0.1:8080/index.php/admin/auth/edituser?uid=' + data["data"][i]["uid"])+'">修改</a>';
	...
	$("#dialog").dialog({
		buttons: {
			"确定,#0059d6,#fff": function() {
				...
				$.ajax({
					type : "post",
					url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/auth/resetuserpassword'); ?>",
					data : { "uid" : uid },
					success : function(data) {
						if(data["code"] > 0 ){
							showTip(data["message"],'success');
							location.href = "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/auth/userlist'); ?>";
						}else{
...
function lock(uid){
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/auth/userlock'); ?>",
		data : { "uid" : uid },
		success : function(data) {
			if(data["code"] > 0 ){
				showTip(data["message"],'success');
...
function unlock(uid){
	$.ajax({
		type : "post",
		url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/Auth/userunlock'); ?>",
		data : { "uid" : uid },
		success : function(data) {
			if(data["code"] > 0 ){
				showTip(data["message"],'success');
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				$(this).dialog('close');
				$.ajax({
				url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/auth/deleteadminuserajax'); ?>",
				data : {"uid" : uid },
				type : "post",
				dataType : "json",
				success : function(data) {
