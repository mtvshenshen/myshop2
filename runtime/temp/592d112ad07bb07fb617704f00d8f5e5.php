<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:42:"template/admin\Order\orderPrintAction.html";i:1552613507;}*/ ?>
<style>
.cell .is_Increment{
	margin-left: 10px;
}
	...
	<div class="modal-body" style="height: 282px; overflow: auto;">
		<form class="form-horizontal">
			<div class="control-group">
				...
				<span><a href="<?php echo __URL('http://127.0.0.1:8080/index.php/admin/express/expresscompany'); ?>" class="goToConfigure" style="text-decoration: underline;">前去配置</a></span>
			</div>
		</form> 
		<div class="ordercontent">
			<div class="data-body-head">
...
function showInvoice() {
	...
	$.ajax({
		url: "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/order/getOrderInvoiceView'); ?>",
		data: { "ids": strIDs },
		async: false, // 让它同步执行
		dataType: "json",
	...
	$.ajax({
		type: "post",
		url: "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/order/getshopinfo'); ?>",
		dataType: "json",
		data: "oper=getType",
...
function invoicePrinPreview(ids) {
	...
	if (ids != "") {
		$("#invoicePrintingURL").attr("href", __URL("http://127.0.0.1:8080/index.php/admin/order/printdeliverypreview?order_ids=" + ids + "&ShopName=" + ShopName + ""));
...
function printPreview() {
	...
	for (var i = 0; i < checks.length; i++) {
		...
		if (check == true) {
			...
			$.ajax({
				url: "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/order/printpreviewvalidate'); ?>",
	...
	if (printQueue.length > 0) {
		$("#expressSinglePrintURl").attr("href", __URL("http://127.0.0.1:8080/index.php/admin/order/printexpresspreview?expressIDs=" + printQueue + "&sid="+$("#expressTemplate").val()));
...
function printShipping(){
	...
	$.ajax({
		type : "post",
		url: "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/order/getShippingList'); ?>",
...
function printpreviewOfInvoice(){
	...
	if (order_ids != "") {
		window.open('<?php echo __URL("http://127.0.0.1:8080/index.php/admin/order/printpreviewOfInvoice?order_ids='+order_ids +'"); ?>');
	}
}
...
function showExpress() {
	...
	$("#print_select_ids").val(strIDs);
	$.ajax({
		url: "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/order/getorderexpresspreview'); ?>",
		data: { "ids": strIDs },
		async: false, // 让它同步执行
		dataType: "json",
		success: function (data) {
	...
	if(express_company_id == null || express_company_id == 0){
		...
		$(".express_company_empty").text(express_name).attr("data_express_company_id",express_company_id);
	}
...
function expressPrinPreview(ids) {
	...
	if(expressno_is_empty){
		$.ajax({
			type : "post",
			url : '<?php echo __URL("http://127.0.0.1:8080/index.php/admin/order/addTmpExpressInformation"); ?>',
		...
		if (print_order_ids != "") {
			window.open(__URL("http://127.0.0.1:8080/index.php/admin/order/printexpresspreview?print_order_ids=" + print_order_ids));
		}
	}
}  
...
function showNotShippingOrderList(){
	...
	$.ajax({
		type : "post",
		url: "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/order/getNotshippedOrderList'); ?>",
		data: { "ids": strIDs },
		async: false, // 让它同步执行
		dataType: "json",
		success: function (data) {
...
$("#PrintOrder").click(function(){
	...
	if (BatchSend.length == 0) {
		...
		window.open(__URL("http://127.0.0.1:8080/index.php/admin/order/printOrder?print_order_ids=" + BatchSend.toString()));
	}
});
</script>