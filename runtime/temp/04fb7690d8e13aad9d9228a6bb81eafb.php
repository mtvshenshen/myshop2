...
<div class="set-style">
	...
	<dl>
		<dt>自动收货时间：</dt>
		<dd>
			<input id="order_auto_delinery" type="number" min="0.00" step="1.00" onkeyup="value=value.replace(/[^\d+(\.\d+)?]/g,'')" value="<?php echo $shopSet['order_auto_delinery']; ?>" class="input-common harf"/><em class="unit">天</em>
	...
	<dl>
		<dt>订单自动关闭时间：</dt>
		<dd>
			<input id="order_buy_close_time" type="number" min="0.00" step="1.00" onkeyup="value=value.replace(/[^\d+(\.\d+)?]/g,'')" value="<?php echo $shopSet['order_buy_close_time']; ?>" class="input-common harf" /><em class="unit">分钟</em>
	...
	<dl style="<?php if(addon_is_exit('NsPickup') != 1): ?> display: none<?php endif; ?>">
		<dt>是否开启买家自提：</dt>
		<dd>
			<div class="controls">
				<p>
	...
	<dl style="<?php if(addon_is_exit('NsO2o') != 1): ?> display: none<?php endif; ?>">
		<dt>是否开启本地配送：</dt>
		<dd>
			<div class="controls">
				<p>
...
function setConfigAjax(){
	...
	if(validation){
		$.ajax({
			type:"post",
			url : "<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/shopset'); ?>",
			data : {
			...
			success : function(data){
				if(data['code'] > 0){
					...
					location.href="<?php echo __URL('http://127.0.0.1:8080/index.php/admin/config/shopset'); ?>";
				}else{
